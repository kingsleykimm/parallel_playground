...
from common import enumerate_grouped_gemms, generate_m_grouped_contiguous, generate_m_grouped_masked, calc_cosine_diff, setup, check_diff, profile, benchmark_no_l2_clear
...
class TESwiglu(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.fc1 = te.GroupedLinear(num_groups, hidden_size, intermediate_size * 2, bias=False, params_dtype = torch.bfloat16)
...
def run(
    *,
    ...
    print(f"Starting run for config num_groups: {num_groups}, gemm_type: {gemm_type}, expected_m_per_group: {expected_m_per_group}, H: {H}, I: {I}")
    device = f"cuda:{rank}"
    ...
    te_model = TESwiglu(hidden_size=H, intermediate_size=I, num_groups=num_groups).to(device)
    ...
    scale_rw = scale_D.T.contiguous()                             # [total_M, I_dim//128]
    ...
    coop_run = lambda : moe_cuda_swiglu_coop(
        input=inputs_q,
        input_scales=inputs_scales,
        gate=gate_q,
        gate_scales=gate_q_scales, up=up_q, up_scales=up_q_scales, gemm_type=gemm_type, D=D, scale_d=scale_D,
    ...
    pingpong_run = lambda : moe_cuda_swiglu_pp(
        input = inputs_q,
        input_scales = inputs_scales,
        gate = gate_q,
    ...
    if do_profile:
        ...
        custom_avg_ms = benchmark_no_l2_clear(coop_run, num_warmup_iters=1, num_iters=20)
        pingpong_avg_ms = benchmark_no_l2_clear(pingpong_run, num_warmup_iters=1, num_iters=20)
        print(f"Coop kernel average time: {custom_avg_ms:.3f} ms for config num_groups: {num_groups}, expected_m_per_group: {expected_m_per_group}, H: {H}, I: {I}")
        print(f"Pingpong kernel average time: {pingpong_avg_ms:.3f} ms for config num_groups: {num_groups}, expected_m_per_group: {expected_m_per_group}, H: {H}, I: {I}")
