...
class TESwigluMLP(nn.Module):
    ...
    def __init__(
        ...
        self.fc1 = te.GroupedLinear(num_groups, hidden_size, intermediate_size * 2, bias=False)
        self.fc2 = te.GroupedLinear(num_groups, intermediate_size, hidden_size, bias=False)
    ...
    def load_weights(
        ...
        with torch.no_grad():
            for i in range(self.fc1.num_gemms):
                getattr(self.fc1, f"weight{i}").copy_(torch.cat([gate_weight[i], up_weight[i]], dim=0))
...
def vllm_forward_fp8(
    ...
    gate_weights_scales = gate_weights_scales.view(local_num_experts, intermediate_size // 128, hidden_size // 128)
    up_weights_scales = up_weights_scales.view(local_num_experts, intermediate_size // 128, hidden_size // 128)
    down_weights_scales = down_weights_scales.view(local_num_experts, hidden_size // 128, intermediate_size // 128)
    ...
    workspace1 = torch.empty(workspace1_shape, dtype = torch.bfloat16, device = device)
    workspace2 = torch.empty(workspace2_shape, dtype = torch.bfloat16, device = device)

    expert_map = torch.arange(local_num_experts, device = device, dtype = torch.int32) + local_rank * local_num_experts
...
def run(
    ...
    quantized_1d = Float8BlockQuantizer(fp8_dtype = TE_DType.kFloat8E4M3, rowwise= True, columnwise = False, block_scaling_dim = 1)
    quantized_2d = Float8BlockQuantizer(fp8_dtype = TE_DType.kFloat8E4M3, rowwise= True, columnwise = True, block_scaling_dim = 2)

    inputs_q_tensor : Float8BlockwiseQTensor = quantized_1d(inputs)
    inputs_q, inputs_scales = inputs_q_tensor._rowwise_data.view(torch.float8_e4m3fn), inputs_q_tensor._rowwise_scale_inv
    up_q_tensor : Float8BlockwiseQTensor = quantized_2d(up_weights_bf)
    up_q, up_q_scales = up_q_tensor._rowwise_data.view(torch.float8_e4m3fn), up_q_tensor._rowwise_scale_inv
    gate_q_tensor : Float8BlockwiseQTensor = quantized_2d(gate_weights_bf)
    gate_q, gate_q_scales = gate_q_tensor._rowwise_data.view(torch.float8_e4m3fn), gate_q_tensor._rowwise_scale_inv
    down_q_tensor : Float8BlockwiseQTensor = quantized_2d(down_weights_bf)
    down_q, down_q_scales = down_q_tensor._rowwise_data.view(torch.float8_e4m3fn), down_q_tensor._rowwise_scale_inv

    


    weights = torch.rand((num_experts), dtype = torch.float32, device = device).repeat(num_tokens_per_dev, 1)
    indices = torch.multinomial(input = weights, num_samples = experts_per_token, replacement = False).to(torch.int32)
    ...
    out_tokens = torch.zeros((num_tokens_per_dev, H), dtype = torch.bfloat16, device = device)

    expert_x = torch.zeros((a2a_handle.max_recv_tokens, H, ), dtype = torch.float8_e4m3fn, device = device)
    expert_x_scales = torch.zeros((H // 128, a2a_handle.max_recv_tokens), dtype = torch.float32, device = device)

    inter_y = torch.zeros((a2a_handle.max_recv_tokens, I), dtype = torch.float8_e4m3fn, device = device)
    inter_y_scales = torch.zeros((I // 128, a2a_handle.max_recv_tokens), dtype = torch.float32, device = device)

    expert_y = torch.zeros((a2a_handle.max_recv_tokens, H), dtype = torch.bfloat16, device = device)
    custom_moe_forward = lambda : naive_moe_forward_fp8(
    ...
    vllm_moe_forward = lambda : vllm_forward_fp8(
        device = device,
        num_experts = num_experts,
    ...
    avg_ms = benchmark_no_l2_clear(custom_moe_forward, num_warmup_iters = 1, num_iters = 20)
