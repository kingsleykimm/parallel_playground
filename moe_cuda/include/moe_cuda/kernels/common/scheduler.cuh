...
static constexpr uint32_t get_num_1d_blocks_per_group() {
  ...
  // usage, min_usage here describe the number of elements needed to load into L2 cache for a single swizzled group
  for (const auto &candidate : {8u, 16u}) {

    uint32_t usage = kIsMulticastOnA ? (BLOCK_N * candidate + constexpr_ti_ceil_div(kNumSMs, candidate) *
                                                                  BLOCK_M) // candidate * num_m_blocks <= kNumSMs
                                     : (BLOCK_M * candidate + constexpr_ti_ceil_div(kNumSMs, candidate) * BLOCK_N);
    if (usage < min_usage) {
      min_usage = usage;
...
template <GemmType kGemmType, uint32_t BLOCK_M, uint32_t BLOCK_N, uint32_t kNumGroups, uint32_t kNumMulticast,
          bool kIsMulticastOnA, uint32_t kNumSMs,
...
struct Scheduler {
  ...
  __device__ __forceinline__ explicit Scheduler(const uint32_t &global_m, const uint32_t &global_n,
                                                const uint32_t &global_k, int *grouped_layout = nullptr) {
  ...
  // this also reorders the m_block_idx, n_block_idx so that it is compatible with the multicast direction passed in
  CUTLASS_DEVICE void get_swizzled_block_idx(const uint32_t &block_idx, uint32_t &m_block_idx, uint32_t &n_block_idx) {

    CUTE_STATIC_ASSERT(kNum1DBlocksPerGroup % kNumMulticast == 0,
    ...
    auto first_block_idx = group_idx * kNum1DBlocksPerGroup; // this the index of the first_block in the
                                                             // primary direction of the group

    auto in_group_idx = block_idx % group_size;
    num_blocks_in_group_swizzle_dir = min(kNum1DBlocksPerGroup,
                                          primary_num_blocks - first_block_idx); // for ragged tile cases at the end

    if (kNumMulticast > 1 && num_blocks_in_group_swizzle_dir % 2 != 0) { // odd sized on the ragged condition
      if (in_group_idx < (num_blocks_in_group_swizzle_dir ^ 1) * secondary_num_blocks) {
        num_blocks_in_group_swizzle_dir =
            num_blocks_in_group_swizzle_dir ^ 1; // truncate for blocks that fit in previously
      } else {
        // subtract by the slice of blocks before it
        in_group_idx = in_group_idx - (num_blocks_in_group_swizzle_dir ^ 1) * secondary_num_blocks;
        first_block_idx +=
            (num_blocks_in_group_swizzle_dir ^ 1); // advance the first_block_idx appropriately by the offset
        num_blocks_in_group_swizzle_dir = 1;
      }
    }

    if constexpr (kIsMulticastOnA) { // N-major, when multicasting A across N dimension
      m_block_idx = in_group_idx / num_blocks_in_group_swizzle_dir;
      n_block_idx = first_block_idx + in_group_idx % num_blocks_in_group_swizzle_dir;
    } else { // M-major
      m_block_idx = first_block_idx + in_group_idx % num_blocks_in_group_swizzle_dir;
  ...
  __device__ __forceinline__ bool get_next_block(uint32_t &m_block_idx, uint32_t &n_block_idx) {
    ...
    if constexpr (kGemmType == GemmType::MGroupedMasked) {
      while (true) {
        ...
        num_m_blocks = ti_ceil_div((uint32_t)__ldg(grouped_layout + cur_group_idx), BLOCK_M);
      ...
      is_peer_cta_alive =
          num_n_blocks % kNumMulticast == 0 || num_m_blocks % kNumMulticast == 0 || (next_block_idx ^ 1) < num_blocks;
  ...
  CUTLASS_DEVICE uint32_t get_global_idx(uint32_t shape, uint32_t block_idx, uint32_t block_size,
    ...
    if constexpr (kGemmType == GemmType::Normal) {
      ...
      // for the experts
      const auto offset = kWithGroupOffset ? max(0, __ldg(grouped_layout + m_block_idx * BLOCK_M)) : 0;
  ...
  CUTLASS_DEVICE bool is_tma_multicast_valid(uint32_t &m_block_idx) {
    ...
    if constexpr (kGemmType == GemmType::Batched || kGemmType == GemmType::MGroupedMasked ||
      ...
      CUTE_STATIC_ASSERT(kGemmType == GemmType::MGroupedContiguous, "Invalid Gemm Type");
      ...
      return group_idx >= 0 && peer_group_idx >= 0 && peer_group_idx == group_idx;
    }
  }

  CUTLASS_DEVICE bool is_computation_valid(uint32_t &block_idx, const uint32_t &block_offset) const {
    if constexpr (kGemmType == GemmType::MGroupedContiguous) {
      auto group_index = __ldg(grouped_layout + block_idx * BLOCK_M + block_offset);
