// // the methods here don't depend on the cuda runtime or cuda::std libraries,
// which is a requirement of any JIT kernel

#pragma once
#include <kittens.cuh>

template <uint32_t kNumBits, typename unit_ptr_t> struct VEC_LOAD {
...
template <int kNumVec, size_t kNumBits> struct VEC_LOAD_PTR {
  static auto base_init() {
...
template <typename from_t, typename to_t> struct VEC_LOAD_CVT {};
...
template <> struct VEC_LOAD_CVT<uint16_t, __half> {
...
template <> struct VEC_LOAD_CVT<uint16_t, __nv_bfloat16> {
  __device__ __forceinline__ static void convert(uint16_t val,
                                                 __nv_bfloat16 *out) {
    *reinterpret_cast<uint16_t *>(out) = val;
  }
};
...
template <> struct VEC_LOAD_CVT<uint32_t, float> {
...
template <> struct VEC_LOAD_CVT<uint32_t, __half> {
...
template <> struct VEC_LOAD_CVT<uint32_t, __nv_bfloat16> {
  __device__ __forceinline__ static void convert(uint32_t val,
                                                 __nv_bfloat16 *outs) {
    *reinterpret_cast<uint32_t *>(outs) = val;
  }
};
...
template <> struct VEC_LOAD_CVT<uint2, float> {
...
template <> struct VEC_LOAD_CVT<uint2, __half> {
...
template <> struct VEC_LOAD_CVT<uint2, __nv_bfloat16> {
  __device__ __forceinline__ static void convert(uint2 val,
                                                 __nv_bfloat16 *outs) {
    *reinterpret_cast<uint2 *>(outs) = val;
  }
};
...
template <> struct VEC_LOAD_CVT<uint4, float> {
...
template <> struct VEC_LOAD_CVT<uint4, __half> {
...
template <> struct VEC_LOAD_CVT<uint4, __nv_bfloat16> {
  __device__ __forceinline__ static void convert(uint4 val,
                                                 __nv_bfloat16 *outs) {
    *reinterpret_cast<uint4 *>(outs) = val;
  }
};
...
template <> struct VEC_LOAD_CVT<__half, uint16_t> {
  __device__ __forceinline__ static void convert(__half *val, uint16_t *outs) {
    *outs = *reinterpret_cast<uint16_t *>(val);
  }
};

template <> struct VEC_LOAD_CVT<__nv_bfloat16, uint16_t> {
  __device__ __forceinline__ static void convert(__nv_bfloat16 *val,
                                                 uint16_t *outs) {
    *outs = *reinterpret_cast<uint16_t *>(val);
  }
};

template <> struct VEC_LOAD_CVT<float, uint32_t> {
  __device__ __forceinline__ static void convert(float *val, uint32_t *outs) {
    *outs = *reinterpret_cast<uint32_t *>(val);
  }
};

template <> struct VEC_LOAD_CVT<__half, uint32_t> {
  __device__ __forceinline__ static void convert(__half *val, uint32_t *outs) {
    *outs = *reinterpret_cast<uint32_t *>(val);
  }
};

template <> struct VEC_LOAD_CVT<__nv_bfloat16, uint32_t> {
  __device__ __forceinline__ static void convert(__nv_bfloat16 *val,
                                                 uint32_t *outs) {
    *outs = *reinterpret_cast<uint32_t *>(val);
  }
...
template <> struct VEC_LOAD_CVT<float, uint2> {
  __device__ __forceinline__ static void convert(float *val, uint2 *out) {
    // for these conversions, we're only doing one value of the to type, since
    // we need to explicitly place into load
...
// 4 bf16 -> uint2
template <> struct VEC_LOAD_CVT<__nv_bfloat16, uint2> {
  __device__ __forceinline__ static void convert(__nv_bfloat16 *val,
                                                 uint2 *out) {
    // for these conversions, we're only doing one value of the to type, since
    // we need to explicitly place into load
...
// 4 bf16 -> uint2
template <> struct VEC_LOAD_CVT<__half, uint2> {
...
template <> struct VEC_LOAD_CVT<float, uint4> {
...
// 4 bf16 -> uint2
template <> struct VEC_LOAD_CVT<__nv_bfloat16, uint4> {
  __device__ __forceinline__ static void convert(__nv_bfloat16 *val,
                                                 uint4 *out) {
    // for these conversions, we're only doing one value of the to type, since
    // we need to explicitly place into load
...
// 4 bf16 -> uint2
template <> struct VEC_LOAD_CVT<__half, uint4> {
...
// template <typename from_t> __forceinline__ __host__ __device__ float
// to_float(from_t val) {
...
//   template <typename to_t> __forceinline__ __host__ __device__ to_t
//   from_float(float val) {
...
__host__ __device__ int ti_ceil_div(TA a, TB b) {
  return (a + b - 1) / b;
}

// align a to b
template <typename TA, typename TB>
__host__ __device__ int ti_align(TA a, TB b) {
  return ti_ceil_div(a, b) * b;
}

template <typename TA, typename TB>
__host__ __device__ constexpr int constexpr_ti_ceil_div(TA a, TB b) {
  return (a + b - 1) / b;
}

// align a to b
template <typename TA, typename TB>
__host__ __device__ constexpr int constexpr_ti_align(TA a, TB b) {
  return constexpr_ti_ceil_div(a, b) * b;
}

template <typename TA, typename TB>
__host__ __device__ constexpr TA constexpr_min(TA a, TB b) {
  return (a <= b) ? a : b;
}
...
// CUTE_HOST_DEVICE constexpr uint32_t swizzle_offset(SwizzleOp swizzle,
// uint32_t logical_idx) {
//     constexpr uint32_t kBytesPerSwizzleUnit = 16;  // 128 bits
//     constexpr uint32_t kElemsPerSwizzleUnit = kBytesPerSwizzleUnit /
//     sizeof(dtype_t);
...
// template <typename SwizzleOp, typename dtype_t>
// CUTE_HOST_DEVICE constexpr uint32_t swizzle_offset(SwizzleOp swizzle,
// uint32_t row, uint32_t col, uint32_t row_stride) {
//     return swizzle_offset<SwizzleOp, dtype_t>(swizzle, row * row_stride +
//     col);
// }
...
// // - Within each warp's region: data arranged for bank-conflict-free WGMMA
// access
...
// CUTE_HOST_DEVICE constexpr uint32_t stsm_wgmma_offset(uint32_t mat_row,
// uint32_t mat_col) {
//     constexpr uint32_t kNumBankGroupBytes = 16;
//     constexpr uint32_t kElemsPerBankGroup = kNumBankGroupBytes /
//     sizeof(dtype_t);  // 8 for bf16 constexpr uint32_t kRowsPerWarp = 16;  //
//     WGMMA M=64 / 4 warps constexpr uint32_t kSwizzleAtomWidth = kSwizzleMode
//     / sizeof(dtype_t);  // 64 for 128-byte swizzle
...
//     uint32_t tile_col = col_in_atom / kElemsPerBankGroup;  // which 8-element
//     tile (0-7 for 64-wide atom) uint32_t elem_in_tile = col_in_atom %
//     kElemsPerBankGroup;  // position within tile (0-7)
...
//     //
//     // The lane_idx in stmatrix corresponds to: row_in_tile + row_tile * 8 +
//     tile_col * 2
//     // (This is the stmatrix thread-to-position mapping)
//     uint32_t stsm_row = row_in_tile + row_tile * 8;  // 0-15 range for rows
//     within warp uint32_t stsm_col = tile_col;  // 0-7 for 8 bank groups
...
// These are the same as torch.finfo(torch.float32) and
...
constexpr DeviceTypeFInfo kernel_finfo_table[] = {
    ...
    // FP32
    {1.1920928955078125e-07f, -3.4028234663852886e+38f,
...
//     __host__ __device__ explicit PatternVisitor(Func_T&& func) :
