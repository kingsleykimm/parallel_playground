...
template <typename lcft>
concept kernel_template = requires {
...
__global__ __launch_bounds__(detail::NUM_THREADS_v<lcft>,
                             detail::NUM_BLOCKS_v<lcft>)
#ifdef KITTENS_BLACKWELL
    __cluster_dims__(detail::CLUSTER_BLOCKS_v<lcft>)
#endif
        void kernel(const __grid_constant__
                    typename lcft::layout::globals globals) {
  static_assert(
      kernel_template<lcft>,
      "lcf kernel template parameter does not satisfy concept requirements");
  using L = typename lcft::layout;
  using CKL = complete_kittens_layout<L>; // complete the layout by filling in
                                          // the optional types with empty
  ...
  static_assert(INPUT_PIPE_STAGES >= 1 && INPUT_PIPE_STAGES <= 16,
                "Invalid number of input pipe stages");
  static_assert(INPUT_PIPE_STAGES * sizeof(input_alloc_block) +
                        sizeof(scratch_alloc_block) <=
                    MAX_SHARED_MEMORY - 1024,
                "Shared memory usage exceeds limits");
  ...
  tensor_allocator<detail::NUM_BLOCKS_v<lcft>, NCTA_TENSOR_ALLOC>
      tensor_alloc{};
#endif
  ...
  input_alloc_block(&input_smem)[INPUT_PIPE_STAGES] =
      alloc.allocate<input_alloc_block, INPUT_PIPE_STAGES>();

  // figure out where we're going to put the finish block
  constexpr int FINISH_BLOCK_OFFSET =
      (MAX_SHARED_MEMORY - 1024) / detail::NUM_BLOCKS_v<lcft> -
      sizeof(finish_block);
  static_assert(FINISH_BLOCK_OFFSET >= 0,
                "Finish block is too large for shared memory.");
  constexpr int NON_FINISH_BLOCK_SPACE =
      FINISH_BLOCK_OFFSET - 1024 -
      sizeof(scratch_alloc_block); // including the losses from alignment
  constexpr int SAFE_STAGES_BETWEEN_BLOCKS =
      NON_FINISH_BLOCK_SPACE / sizeof(input_alloc_block) < INPUT_PIPE_STAGES
          ? NON_FINISH_BLOCK_SPACE / sizeof(input_alloc_block)
          : INPUT_PIPE_STAGES;
  finish_block(*finish_smem) = reinterpret_cast<finish_block *>(
      (((uint64_t)&__shm[0] + FINISH_BLOCK_OFFSET) / 1024) * 1024); // alignment
  ...
  // Initialize semaphores. This is constant for all two-stage producer-consumer
  // kernels.
  __shared__ kittens::semaphore inputs_arrived[INPUT_PIPE_STAGES],
      inputs_finished[INPUT_PIPE_STAGES];
  __shared__ kittens::semaphore finish_finished;
  uint32_t semaphore_bitfield =
      0xFFFF0000; // ***_finished phase bits start as 1s, ***_arrived phase bits
                  // start as 0s
  common_state common;

  if (warpid() >= NUM_CONSUMER_WARPS) { // code path for producer warps
    using producers = group<NUM_PRODUCER_WARPS>;
    if (warpid() == NUM_CONSUMER_WARPS) { // a single warp (in fact a single
                                          // thread) does these.
      for (int i = 0; i < INPUT_PIPE_STAGES; i++) {
        init_semaphore(inputs_arrived[i],
                       detail::PRODUCER_BARRIER_ARRIVALS_v<lcft>,
                       0); // needs to wait on each producer warp
        init_semaphore(
            inputs_finished[i], detail::CONSUMER_BARRIER_ARRIVALS_v<lcft>,
            0); // needs to wait on one thread from each consumer warp
      }
      init_semaphore(
          finish_finished, detail::CONSUMER_BARRIER_ARRIVALS_v<lcft>,
          0); // consumer warps must say they are done with the finish block
    }
    // all warps must arrive here, confirming semaphore initialization is
    // visible to all threads.
    if constexpr (detail::CLUSTER_BLOCKS_v<lcft> > 1)
    ...
    for (int task_iter = 0; true; task_iter++) {
      int num_iters = -1;
#ifdef KITTENS_BLACKWELL
      common_setup_args<L> unif{common,  task_iter,     num_iters,
                                globals, *scratch_smem, tensor_alloc};
#else
      common_setup_args<L> unif{common, task_iter, num_iters, globals,
                                *scratch_smem};
#endif
      ...
      for (load_iter = 0;
           load_iter < SAFE_STAGES_BETWEEN_BLOCKS && load_iter < num_iters;
           load_iter++) { // fill the pipeline
        ...
        lcft::producer::load({p_state, *input_smem[input_ring],
                              inputs_arrived[input_ring], load_iter, unif});
        input_ring = ring_advance<INPUT_PIPE_STAGES>(input_ring);
      }
      wait(finish_finished,
           (task_iter % 2) ^ 1); // wait for consumer to finish their finish
                                 // stage before we can do the rest.
      ...
      producers::sync(
          13); // producer warps must finish before consumer warps can proceed
    } // task iter loop
  } // producer warpgroup
...
#pragma unroll CONSUMER_UNROLL_VALUE
      ...
      for (int it = 0; it < num_iters; it++) {
        wait(inputs_arrived[input_ring],
             get_phasebit<0>(
                 semaphore_bitfield,
                 input_ring)); // wait for memory to arrive, phase changes at
                               // half the rate of the ring
        update_phasebit<0>(semaphore_bitfield, input_ring);
        lcft::consumer::compute({c_state, *input_smem[input_ring],
                                 inputs_finished[input_ring], it, unif});
        input_ring = ring_advance<INPUT_PIPE_STAGES>(input_ring);
      } // work loop
      consumers::sync(14); // cannot overwrite finish block until all consumer
                           // warps are done.
  ...
  // all warps must arrive here, confirming semaphore initialization is visible
  // to all threads.
  if constexpr (detail::CLUSTER_BLOCKS_v<lcft> > 1)
    everyone::tma::cluster::sync();
#ifdef KITTENS_BLACKWELL
