...
// TODO : implement GemmType::Batched Support as well, and implement the
// kNumMergeStages (increase BLOCK_K), excessive stages can cause slowdowns

template <Major kMajorA, Major kMajorB, uint32_t SHAPE_M, uint32_t SHAPE_N,
          uint32_t SHAPE_K, uint32_t kNumGroups, uint32_t BLOCK_M,
          uint32_t BLOCK_N, uint32_t BLOCK_K_, uint32_t kSwizzleAMode,
          uint32_t kSwizzleBMode, uint32_t kSwizzleDMode, uint32_t kNumStages_,
          uint32_t kNumTMAThreads, uint32_t kNumMathThreads,
          uint32_t kNumTMAMulticast, bool kIsTMAMulticastOnA, uint32_t kNumSMs,
...
__global__ void __launch_bounds__(kNumTMAThreads + kNumMathThreads, 1)
    sm90_bf16_gemm(__grid_constant__ const cute::TmaDescriptor a_tensor_map,
                   ...
                   __grid_constant__ const cute::TmaDescriptor d_tensor_map,
                   uint32_t shape_m, uint32_t shape_n, uint32_t shape_k,
                   int *grouped_layout) {

  constexpr uint32_t kMergeStages =
      kNumStages_ >= 10 && kGemmType == GemmType::Normal &&
      kMajorA == Major::K && kMajorB == Major::K && kNumMathThreads == 128;
  ...
  constexpr uint32_t SMEM_A_SIZE_PER_STAGE =
      sizeof(__nv_bfloat16) * BLOCK_M * BLOCK_K_;
  constexpr uint32_t SMEM_B_SIZE_PER_STAGE =
      sizeof(__nv_bfloat16) * BLOCK_N * BLOCK_K_;
  constexpr uint32_t SMEM_D_SIZE =
      constexpr_ti_align(sizeof(cd_dtype_t) * BLOCK_M * BLOCK_N, 1024);

  static constexpr uint32_t WGMMA_M_SIZE_PER_STAGE =
      WGMMA::M * BLOCK_K_ * sizeof(__nv_bfloat16);

  // for the cases where BLOCK_M << WGMMA::M, what we will actually use could be
  // a lot larger, we just need to make sure the smem size is enough so we don't
  // go out of bounds
  static_assert(WGMMA_M_SIZE_PER_STAGE <=
                SMEM_A_SIZE_PER_STAGE + SMEM_B_SIZE_PER_STAGE * kNumStages_);

  auto smem_d = reinterpret_cast<cd_dtype_t *>(smem);

  auto smem_a = PatternVisitor([&](const uint32_t &stage_idx) {
    return reinterpret_cast<__nv_bfloat16 *>(smem + SMEM_D_SIZE +
                                             SMEM_A_SIZE_PER_STAGE * stage_idx);
  });
  auto smem_b = PatternVisitor([&](const uint32_t &stage_idx) {
    return reinterpret_cast<__nv_bfloat16 *>(
        smem + SMEM_D_SIZE + SMEM_A_SIZE_PER_STAGE * kNumStages_ +
        SMEM_B_SIZE_PER_STAGE * stage_idx);
  });

  uint8_t *smem_barrier_offset =
      smem + (SMEM_A_SIZE_PER_STAGE + SMEM_B_SIZE_PER_STAGE) * kNumStages_ +
      SMEM_D_SIZE;

  using TMABarrier = cutlass::arch::ClusterTransactionBarrier;
  using MathBarrier = cutlass::arch::ClusterBarrier;
  auto math_barriers = PatternVisitor([&](const uint32_t &stage_idx) {
    return reinterpret_cast<MathBarrier *>(smem_barrier_offset +
                                           sizeof(MathBarrier) * stage_idx);
  });
  auto tma_barriers = PatternVisitor([&](const uint32_t &stage_idx) {
    return reinterpret_cast<TMABarrier *>(
        smem_barrier_offset + (kNumStages_ + stage_idx) * sizeof(MathBarrier));
  });
  ...
  if (warp_id >= kNumMathThreads / 32) {
    ...
    while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
      if (warp_id == kNumMathThreads / 32 + 2 && lane_predicate) {
        ...
        int numAMulticast =
            (kIsTMAMulticastOnA && is_multicast_valid) ? kNumTMAMulticast : 1;
        int numBMulticast =
            (!kIsTMAMulticastOnA && is_multicast_valid) ? kNumTMAMulticast : 1;
        // int numAMulticast = 1;
        // int numBMulticast = 1;
        for (int k_block_idx = 0; k_block_idx < num_k_blocks;
          ...
          // if the gemm type is batched or masked, then the A descriptor will
          // be 3D. We don't use 3D descriptors for the B tensor because it is
          // usually a weight tensor. In grouped gemms, the scheduler should be
          // able to handle most edge cases
          auto m_idx = scheduler.template get_global_idx<false>(
              shape_m, m_block_idx, BLOCK_M);

          // only when kMajorB, should we stride advance. if kMajorB ==
          // Major::MN, we're doing Normal GEMM only when it is normal can we
          // have a non k major

          auto k_a_idx =
              scheduler.template get_global_idx < kMajorA ==
              Major::MN > (shape_k, k_block_idx, BLOCK_K_, m_block_idx);
          auto k_b_idx =
              scheduler.template get_global_idx < kMajorB ==
              Major::MN > (shape_k, k_block_idx, BLOCK_K_, m_block_idx);
          ...
          if constexpr (kGemmType == GemmType::MGroupedMasked ||
            ...
            tma_copy<BLOCK_K_, BLOCK_M, kSwizzleAMode, __nv_bfloat16, 3>(
                &a_tensor_map, &barrier, smem_a[stage_idx], k_a_idx, m_idx,
                numAMulticast, scheduler.cur_group_idx);
          } else {
            if constexpr (kMajorA == Major::K) {
              tma_copy<BLOCK_K_, BLOCK_M, kSwizzleAMode>(
                  &a_tensor_map, &barrier, smem_a[stage_idx], k_a_idx, m_idx,
                  numAMulticast);
            } else if constexpr (kMajorA == Major::MN) {
              tma_copy<BLOCK_M, BLOCK_K_, kSwizzleAMode>(
                  &a_tensor_map, &barrier, smem_a[stage_idx], m_idx, k_a_idx,
                  numAMulticast);
            }
          }

          if constexpr (kGemmType == GemmType::MGroupedMasked) {
            // for mgroupedmasked, in some edges the BLOCK_N will not be
            // divisible by the global N, which causes incorrect weight loads
            // only Major::K
            auto n_idx = scheduler.template get_global_idx<false>(
                shape_n, n_block_idx, BLOCK_N);
            tma_copy<BLOCK_K_, BLOCK_N, kSwizzleBMode, __nv_bfloat16, 3>(
                &b_tensor_map, &barrier, smem_b[stage_idx], k_b_idx, n_idx,
                numBMulticast, scheduler.cur_group_idx);
          } else {
            auto n_idx = scheduler.template get_global_idx<true>(
                shape_n, n_block_idx, BLOCK_N, m_block_idx);
            if constexpr (kMajorB == Major::K) {
              tma_copy<BLOCK_K_, BLOCK_N, kSwizzleBMode>(
                  &b_tensor_map, &barrier, smem_b[stage_idx], k_b_idx, n_idx,
                  numBMulticast);
            } else if constexpr (kMajorB == Major::MN) {
              tma_copy<BLOCK_N, BLOCK_K_, kSwizzleBMode>(
                  &b_tensor_map, &barrier, smem_b[stage_idx], n_idx, k_b_idx,
                  numBMulticast);
            }
          }
          ...
          tma_barriers[write_index]->arrive_and_expect_tx(
    ...
    auto empty_barrier_arrive = [&]() {
      if constexpr (kNumTMAMulticast == 1) {
        ...
        lane_id < kNumTMAMulticast ? math_barriers[stage_idx]->arrive(peer_cta)
                                   : void();
      }
    };
    ...
    auto desc_a = create_wgmma_desc<Major::K, BLOCK_M, BLOCK_K_, kSwizzleAMode,
                                    __nv_bfloat16>(
        smem_a[0] + math_wg_idx * WGMMA::M * BLOCK_K_);
    auto desc_b = create_wgmma_desc<Major::K, BLOCK_N, BLOCK_K_, kSwizzleBMode,
                                    __nv_bfloat16>(smem_b[0]);
    ...
    constexpr uint32_t TMA_D_BLOCK_N =
        kSwizzleDMode == 0 ? BLOCK_N : kSwizzleDMode / sizeof(cd_dtype_t);
    ...
    static_assert(TMA_D_BLOCK_N % 8 == 0,
                  "unaligned tma stores for wgmma accumulator layout");
    static_assert(BLOCK_N / TMA_D_BLOCK_N <= 32, "too many tma stores");

    constexpr uint32_t kNumWGMMAStoreThreads =
        (BLOCK_M / WAVE_BLOCK_M) * kNumMathThreads;
    bool do_wgmma_store =
        BLOCK_M >= 64 || (warp_id < kNumWGMMAStoreThreads / 32);
    while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
...
#pragma unroll 8
        for (int k_block_idx = 0; k_block_idx < num_k_blocks;
          ...
          const auto &smem_a_desc_lo =
              a_desc_lo + ((stage_idx * SMEM_A_SIZE_PER_STAGE) >> 4u);
          const auto &smem_b_desc_lo =
              b_desc_lo + ((stage_idx * SMEM_B_SIZE_PER_STAGE) >> 4u);
#pragma unroll
          for (uint32_t i = 0; i < WGMMA::kNumAccum * (BLOCK_M / WAVE_BLOCK_M);
               i++) {
            ...
            for (int k = 0; k < BLOCK_K_ / WGMMA::K; k++) {
              if constexpr (kMajorA == Major::MN) {
                const uint32_t swizzle_atom_offset =
                    m_offset /
                    TMA_A_BLOCK_M; // when MN-major, this will tell how many
                                   // atoms to advance in MN direction
                desc_a.reg32_[0] =
                    smem_a_desc_lo +
                    (((swizzle_atom_offset * 8 * kSwizzleAMode +
                       k * WGMMA::K *
                           get_gmma_desc_stride_k<kMajorA, BLOCK_M, BLOCK_K_,
                                                  kSwizzleAMode,
                                                  __nv_bfloat16>() *
              ...
              desc_b.reg32_[0] =
                  smem_b_desc_lo +
                  (((k * WGMMA::K *
                     get_gmma_desc_stride_k<kMajorB, BLOCK_N, BLOCK_K_,
                                            kSwizzleBMode, __nv_bfloat16>()) *
                    sizeof(__nv_bfloat16)) >>
                   4u);
...
#pragma unroll
      ...
      if constexpr (cute::is_same_v<cd_dtype_t, __nv_bfloat16>) {
        ...
        for (uint32_t wave = 0; wave < NUM_WAVES; wave++) {
          ...
          store_accum_to_swizzled_smem<BLOCK_N, BLOCK_M, kSwizzleDMode,
                                       WGMMA_M_PER_WARP>(
              shifted_accum, smem_ptr, warp_id, lane_id);
        }
...
#pragma unroll
          for (uint32_t i = 0; i < WGMMA::kNumAccum / 4; i++) {
            ...
            float2 *smem_0 = reinterpret_cast<float2 *>(
                smem_d +
                (offset + warp_id * WGMMA_M_PER_WARP + lane_id / 4) * BLOCK_N +
                i * 8 + ((lane_id % 4) * 2));
            float2 *smem_1 = reinterpret_cast<float2 *>(
                smem_d +
                (offset + warp_id * WGMMA_M_PER_WARP + lane_id / 4 + 8) *
                    BLOCK_N +
                i * 8 + ((lane_id % 4) * 2));

            st_shared(smem_0, make_float2(shifted_accum[i * 4 + 0],
                                          shifted_accum[i * 4 + 1]));
            st_shared(smem_1, make_float2(shifted_accum[i * 4 + 2],
                                          shifted_accum[i * 4 + 3]));
          }
        }
      ...
      if (threadIdx.x < BLOCK_N / TMA_D_BLOCK_N) {
        ...
        if constexpr (kGemmType == GemmType::Batched ||
          ...
          cute_tma_t::copy(&d_tensor_map, smem_d + BLOCK_M * n_block_offset,
                           n_block_idx * BLOCK_N + n_block_offset,
                           scheduler.template get_global_idx<false>(
                               shape_m, m_block_idx, BLOCK_M),
