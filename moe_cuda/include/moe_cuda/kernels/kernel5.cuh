...
template <int _M, int _I, int _H, int _BM, int _BN, int NUM_GROUPS,
...
struct globals {
  ...
  static constexpr bool kIsUniformScales = (128 % BN) == 0;
  ...
  using down_tile = st_fp8e4m3<BN, 128>;

  using inter_tile = st_fp8e4m3<BM, 128>;
  ...
  struct consumer_state {
    ...
    accum_tile<float> down_accum;
  };

  __device__ inline void scheduler(common_state &common, int &task_iter,
                                   int &num_inter_iters,
                                   int &num_final_iters) const {
    int Rblocks = constexpr_ti_ceil_div(
            _M, 64), // two warpgroups that each take BM = 64 tile
        Iblocks = constexpr_ti_ceil_div(_I, 128), // tile of intermediate
        CBlocks =
            constexpr_ti_ceil_div(_H, BN); // tile of output hidden dimensions
    ...
    num_inter_iters = _H / 128;
    num_final_iters = _I / 128;
...
kernel5(const globals<_M, _I, _H, _BM, _BN, NUM_GROUPS, NUM_CONSUMER_WARPS_,
                      NUM_PRODUCER_WARPS_, NUM_STAGES_, KERNEL_SMEM_SIZE,
                      GEMM_TYPE, c_dtype, SUPER_N> &G) {

  using GLOBALS = globals<_M, _I, _H, _BM, _BN, NUM_GROUPS, NUM_CONSUMER_WARPS_,
                          NUM_PRODUCER_WARPS_, NUM_STAGES_, KERNEL_SMEM_SIZE,
                          GEMM_TYPE, c_dtype, SUPER_N>;
  ...
  constexpr int shape_k_inter = constexpr_ti_ceil_div(_H, 128);
  constexpr int shape_n_sf_inter = constexpr_ti_ceil_div(_I, 128);
  ...
  __shared__ semaphore first_inputs_arrived[NUM_STAGES_];
  __shared__ semaphore first_inputs_finished[NUM_STAGES_];
  __shared__ semaphore second_inputs_arrived[NUM_STAGES_];
  __shared__ semaphore second_inputs_finished[NUM_STAGES_];
  __shared__ semaphore finish_finished;
  __shared__ semaphore consumer_can_start[2];
  uint32_t first_semaphore_bitfield = 0xFFFF0000;
  uint32_t second_semaphore_bitfield = 0xFFFF0000;
...
#pragma unroll
    for (int stage = 0; stage < GLOBALS::NUM_STAGES; stage++) {
      init_semaphore(first_inputs_arrived[stage], 1, 0);
      init_semaphore(first_inputs_finished[stage], NUM_CONSUMER_WARPS_ / 2, 0);
      init_semaphore(second_inputs_arrived[stage], 1, 0);
      init_semaphore(second_inputs_finished[stage], NUM_CONSUMER_WARPS_ / 2, 0);
    }
  ...
  if (warpid() >= NUM_CONSUMER_WARPS_) {
    ...
    for (int task_iter = 0; true; task_iter++) {
      ...
      int num_inter_iters, num_final_iters = 0;
      G.scheduler(common, task_iter, num_inter_iters, num_final_iters);
      if (num_inter_iters < 0 || num_final_iters) {
        break;
      }
      ...
      int load_iter;
      for (load_iter = 0; load_iter < num_inter_iters; load_iter++) {
        wait(first_inputs_finished[input_ring],
             get_phasebit<1>(first_semaphore_bitfield, input_ring));
        update_phasebit<1>(first_semaphore_bitfield, input_ring);
        if (warpgroup::laneid() == 0) {
          ...
          const int b_element_row = G.template get_global_idx<true>(
              _H, common.coord.y * 128, common, common.m_block_idx);
