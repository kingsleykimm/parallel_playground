...
struct matmul_template {
  ...
  __device__ static int get_global_idx(int num_tiles, int tile_idx,
                                       uniform_args<layout> args,
                                       int m_block_idx = 0) {
    if (isGroupedContiguous) {
      const auto offset = kWithGroupOffset
                              ? max(0, __ldg(args.globals.grouped_layout +
                                             m_block_idx * layout::BM))
                              : 0;
      return offset * num_tiles + tile_idx;
    } else if constexpr (isGroupedMasked) {
      const auto offset = kWithGroupOffset ? args.globals.cur_group_idx : 0;
      return offset * num_tiles + tile_idx;
    } else {
      // normal gemm
      return tile_idx;
  ...
  is_computation_valid(uniform_args<layout> args) {
    if constexpr (isGroupedContiguous) {
      ...
      auto group_index =
          __ldg(args.globals.grouped_layout + args.common.coord.x * layout::BM +
                warpgroup::groupid() * 64);
      return group_index >= 0;
    } else if constexpr (isGroupedMasked) {
      auto group_limit =
          __ldg(args.globals.grouped_layout + args.globals.cur_group_idx);
      return args.common.coord.x * layout::BM + warpgroup::groupid() * 64 <
             group_limit;
    }
  }
  ...
  __device__ static inline void common_setup(common_setup_args<layout> args) {
    if constexpr (DEBUG) {
      if (threadIdx.x == 0 && blockIdx.x == 0) {
        printf("setting up common coords \n");
      }
    }
    auto &cur_group_idx = args.globals.cur_group_idx;
    auto &current_m_cumsum = args.globals.current_m_cumsum;

    args.common.shape_k_scales = ti_ceil_div(args.globals.A.rows(), layout::BK);
    args.common.shape_n_sfb = ti_ceil_div(args.globals.C.cols(), layout::BK);
    ...
    if constexpr (isGroupedMasked) {
      while (true) {
        if (cur_group_idx == NUM_GROUPS) {
          args.num_iters = -1;
          return;
        }

        int num_m_blocks = ti_ceil_div(
            __ldg(args.globals.grouped_layout + cur_group_idx), layout::BM);
      ...
      task_id -= current_m_cumsum * Cblocks;
    }
    ...
    int id = warpgroup::groupid() == NUM_CONSUMER_WARPS / 4
                 ? 0
                 : warpgroup::groupid();
    int m_block_idx = args.common.coord.x * NUM_TILES + id;

    args.common.coord = {
        get_global_idx<isGroupedMasked>(
            Rblocks, args.common.coord.x * NUM_TILES + id, args),
        get_global_idx<true>(Cblocks, args.common.coord.y, args, m_block_idx)};
    // otherwise, we don't need to offset the weights
  }
  ...
  struct consumer {
    ...
    __device__ static void compute(consumer_compute_args<layout> args) {
      ...
      if (is_computation_valid(args)) {
        ...
        auto previous_group_offset = get_global_idx<true>(
            args.common.shape_k_scales * args.common.shape_n_sfb, 0, args,
            args.common.coord.x);
        float *local_sfb =
            args.globals.scale_b.raw_ptr + previous_group_offset +
