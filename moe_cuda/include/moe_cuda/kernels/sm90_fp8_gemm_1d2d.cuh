...
// template <class SA_layout, class SFA_layout, class SB_layout, class SFB_Layout, class SD_layout, uint32_t kStages>
// struct SharedStorage {

//   cute::array_aligned<__nv_fp8_e4m3, cute::cosize_v<SA_layout>, 128> sA;
//   cute::array_aligned<float, cute::cosize_v<SFA_layout>, 128> sSFA; // (1, bM)
...
template <uint32_t kNumGroups, int SHAPE_M, int SHAPE_N, int SHAPE_K, uint32_t bM, uint32_t bN, uint32_t bK,
          Major kSFBMajor, uint32_t kNumMMAThreads, uint32_t kNumTMAThreads, bool kIsTMAMulticastA,
          uint32_t kNumTMAMulticast, GemmType kGemmType, uint32_t kNumSMs, uint32_t kSwizzleAMode,
          uint32_t kSwizzleBMode, uint32_t kSwizzleDMode, uint32_t kNumStages, bool kIsBatched>
__global__ __launch_bounds__(kNumMMAThreads + kNumTMAThreads,
                             1) void sm90_fp8_gemm_1d2d(uint32_t M, uint32_t N, uint32_t K,
                                                        ...
                                                        CUTE_GRID_CONSTANT const cute::TmaDescriptor d_tensor_map,
                                                        float *sfb, int *grouped_layout) {

  // set MNK to compiled dims if possible - this is good for repetitive linear weights, where N and K are fixed
  ...
  CUTE_STATIC_ASSERT(cute::ceil_div(bK, bN) == 1 || cute::ceil_div(bN, bK) <= 2,
                     "Current FP8 GEMM kernel only supports up to two different b scales");
  CUTE_STATIC_ASSERT(bK == 128, "fp8_gemm: Only per-channel 128 block scaling is supported");
  CUTE_STATIC_ASSERT(bM % WGMMA::M == 0, "fp8_gemm: Invalid bM config for FP8 GEMM");

  CUTE_STATIC_ASSERT(kNumTMAMulticast <= 2, "Fp8_gemm kernel only supports up to 2 Multicast size for scheduler");
  ...
  constexpr uint32_t smem_d_size = constexpr_ti_align(bM * bN * sizeof(__nv_bfloat16), 1024);
  constexpr uint32_t WGMMA_A_SIZE_PER_STAGE = WGMMA::M * bK * sizeof(__nv_fp8_e4m3);
  constexpr uint32_t A_SIZE_PER_STAGE = bM * bK * sizeof(__nv_fp8_e4m3); // already aligned to 128
  constexpr uint32_t B_SIZE_PER_STAGE = bN * bK * sizeof(__nv_fp8_e4m3);
  constexpr uint32_t SFA_SIZE_PER_STAGE = constexpr_ti_align(bM * sizeof(float), 128); // align to 128
  const uint32_t SFB_SIZE_PER_STAGE = ti_align(shape_k_scales * (kIsUniformBScales ? 1 : 2) * sizeof(float),
                                               16); // need to align for barrier next
  CUTE_STATIC_ASSERT(WGMMA_A_SIZE_PER_STAGE <= A_SIZE_PER_STAGE + B_SIZE_PER_STAGE,
  ...
  __nv_fp8_e4m3 *sB = reinterpret_cast<__nv_fp8_e4m3 *>(sMem + smem_d_size + A_SIZE_PER_STAGE * kNumStages);
  constexpr uint32_t SF_OFFSET = smem_d_size + (A_SIZE_PER_STAGE + B_SIZE_PER_STAGE) * kNumStages;
  float *sSFA = reinterpret_cast<float *>(sMem + SF_OFFSET);
  float *sSFB = reinterpret_cast<float *>(sMem + SF_OFFSET + (SFA_SIZE_PER_STAGE)*kNumStages);
  const uint32_t barrier_offset =
      smem_d_size + (A_SIZE_PER_STAGE + B_SIZE_PER_STAGE + SFA_SIZE_PER_STAGE + SFB_SIZE_PER_STAGE) * kNumStages;
  ProducerBarrier *tma_barriers = reinterpret_cast<ProducerBarrier *>(sMem + barrier_offset);
  ConsumerBarrier *mma_barriers =
      reinterpret_cast<ConsumerBarrier *>(sMem + barrier_offset + kNumStages * sizeof(ProducerBarrier));
  // barriers
  // configs
...
#pragma unroll
  ...
  auto scheduler =
      Scheduler<kGemmType, bM, bN, kNumGroups, kNumTMAMulticast, kIsTMAMulticastA, kNumSMs>(M, N, K, grouped_layout);

  // set up number of registers here:
  constexpr uint32_t kNumTMARegisters = 40;
  constexpr uint32_t kNumMMARegisters = kNumMMAThreads == 128 ? 248 : 232;
  constexpr uint64_t kTransactionBytes = sizeof(__nv_fp8_e4m3) * (bM * bK + bN * bK) + sizeof(float) * bM;
  ...
  auto empty_barrier_arrive = [&]() {
    if constexpr (kNumTMAMulticast == 1) {
      ...
      auto peer_cta = scheduler.is_peer_cta_alive ? lane_idx : cute::block_rank_in_cluster();
      // per warp, issue multicast predicated for multicast size
      lane_idx < kNumTMAMulticast ? mma_barriers[stage_idx].arrive(peer_cta) : void();
    }
  };
  ...
  if (threadIdx.x >= kNumMMAThreads) {
    ...
    while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
      if (warpIdx == kNumMMAThreads / 32 + 2 && lane_predicate) {
        ...
        int numAMulticast = (kIsTMAMulticastA && is_multicast_valid) ? kNumTMAMulticast : 1;
        int numBMulticast = (!kIsTMAMulticastA && is_multicast_valid) ? kNumTMAMulticast : 1;
        // int numAMulticast = 1;
        // int numBMulticast = 1;
        for (int k_block_idx = 0; k_block_idx < num_k_blocks; advance_pipeline(k_block_idx)) {
          ...
          // TODO: write the same 3d TMA descriptor handling that we did for b16 temm
          if constexpr (kGemmType == GemmType::MGroupedMasked) {
            tma_copy<bK, bM, kSwizzleAMode, __nv_fp8_e4m3, 3>(
                &a_tensor_map, &barrier, cur_stage_sA, k_block_idx * bK,
                scheduler.template get_global_idx<kWithGroupOffsetA>(M, m_block_idx, bM), numAMulticast,
                scheduler.cur_group_idx);

            // sfa can still be 2d, since it's MN-major
            tma_copy<bM, bK, 0>(&sfa_tensor_map, &barrier, cur_stage_sSFA, m_block_idx * bM,
                                scheduler.template get_global_idx<true>(shape_k_scales, k_block_idx, 1), numAMulticast);
            ...
            tma_copy<bK, bN, kSwizzleBMode, __nv_fp8_e4m3, 3>(
                ...
                scheduler.template get_global_idx<false>(N, n_block_idx, bN), numBMulticast, scheduler.cur_group_idx);
          } else {
            tma_copy<bK, bM, kSwizzleAMode>(&a_tensor_map, &barrier, cur_stage_sA, k_block_idx * bK,
                                            scheduler.template get_global_idx<kWithGroupOffsetA>(M, m_block_idx, bM),
                                            numAMulticast);

            tma_copy<bM, bK, 0>(&sfa_tensor_map, &barrier, cur_stage_sSFA, m_block_idx * bM,
                                scheduler.template get_global_idx<kWithGroupOffsetA>(shape_k_scales, k_block_idx, 1),
                                numAMulticast);
            // to break down the k_global_idx a little more (I needed to):
            // per group (or batch) there is an array of

            tma_copy<bK, bN, kSwizzleBMode>(&b_tensor_map, &barrier, cur_stage_sB, k_block_idx * bK,
                                            scheduler.template get_global_idx<true>(N, n_block_idx, bN, m_block_idx),
                                            numBMulticast);
          }
    ...
    auto desc_a = create_wgmma_desc<Major::K, bM, bK, kSwizzleAMode, __nv_fp8_e4m3>(sA + math_wg_idx * WGMMA::M * bK);
    auto desc_b = create_wgmma_desc<Major::K, bN, bK, kSwizzleBMode, __nv_fp8_e4m3>(sB);
    ...
    while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
      ...
      // if (cute::thread0()) {
      //   printf("m block idx : %d, n block idx : %d, group_idx : %d", m_block_idx, n_block_idx,
      //   scheduler.cur_group_idx);
      // }
      ...
      uint32_t num_sfb = shape_k_scales * (num_full_iters > num_former_iters ? 2 : 1);
      ...
      if (threadIdx.x >= 32) {
        ...
        auto previous_group_offset =
            scheduler.template get_global_idx<true>(shape_k_scales * shape_n_sfb, 0, 0, m_block_idx);
        ...
        auto local_sfb = sfb + previous_group_offset + (n_block_idx * bN / 128) * stride_n_sfb;
        // grid-stride loop inside kNumMMAThreads - 32 to load in the
        // scale factors

#pragma unroll
        for (int i = threadIdx.x - 32; i < (int)num_sfb; i += kNumMMAThreads - 32) {
          st_shared(sSFB + i,
                    __ldg(i < shape_k_scales ? local_sfb + stride_k_sfb * i
                                             : local_sfb + stride_n_sfb + (i - shape_k_scales) * stride_k_sfb));
        }
      ...
      CUTE_STATIC_ASSERT(bM % WGMMA::M == 0, "fp8_gemm.cu: bM should be divisble by WGMMA::M for "
                                             "accumulations, in order to proper wave loops");
      CUTE_STATIC_ASSERT(bM >= 64 || kNumMMAThreads == 128, "fp8_gemm.cu: if bM <= 64 (WGMMA::M) atom size, only "
      ...
      static_assert(bM >= 64 || kNumMMAThreads == 128, "Only one math warp group for `BLOCK_M < 64`");
      constexpr uint32_t kNumWGMMAStoreThreads = WAVE_BLOCK_M * (128 / WGMMA::M);
      const bool do_wgmma_store = bM >= WGMMA::M || warpIdx < kNumWGMMAStoreThreads / 32;
      ...
      if (scheduler.is_computation_valid(m_block_idx, wg_block_offset)) {
        ...
        const bool kShouldOptimize = !kIsUniformBScales && bK / cute::gcd(bK, bN) <= 4;
        const uint32_t kGap = cute::gcd(bK, bN) / 8;
        const uint32_t kEnd = kShouldOptimize ? bK / 8 : 0;

        float accum[WGMMA::kNumAccum];
        dispatch_num_former_iters<0, kGap, kEnd>(kShouldOptimize ? num_former_iters : 0, [&](int _) {
#pragma unroll 8
          for (int k_block_idx = 0; k_block_idx < num_k_blocks; advance_pipeline(k_block_idx)) {

            int read_index = stage_idx;

            // advance smem_addr of a_desc, b_desc to the current stage index
            const auto &a_desc_base_lo = a_desc_lo + read_index * ((bM * bK) / 16); // gmma_desc requires shift by 4
            const auto &b_desc_base_lo = b_desc_lo + read_index * ((bN * bK) / 16);
...
#pragma unroll
            for (uint32_t local_idx = 0; local_idx < bM / WAVE_BLOCK_M; ++local_idx) {
              ...
              // sSFA is stored in MN-major, so to advance a k-block, we have to add the number of sclaes in this block
              auto cur_k_block_sfa = sSFA + bM * read_index; // shape: (bM)
              const auto &a_scale_0 = do_wgmma_store ? ld_shared(cur_k_block_sfa + r_0 + m_offset) : 0;
              const auto &a_scale_1 = do_wgmma_store ? ld_shared(cur_k_block_sfa + r_1 + m_offset) : 0;
...
#pragma unroll
              ...
              for (int wgmma_k_idx = 0; wgmma_k_idx < bK / WGMMA::K; wgmma_k_idx++) {
                // move the start_addr of the gmma_desc
                desc_a.reg32_[0] = a_desc_base_lo + ((wgmma_k_idx * WGMMA::K + m_offset * bK) >> 4);
                desc_b.reg32_[0] = b_desc_base_lo + ((WGMMA::K * wgmma_k_idx) >> 4);
...
#pragma unroll
              ...
              float scale_0_0 = a_scale_0 * b_scale_0, scale_1_0 = a_scale_1 * b_scale_0;
              float scale_0_1, scale_1_1;
              if constexpr (!kIsUniformBScales) {
                scale_0_1 = a_scale_0 * b_scale_1, scale_1_1 = a_scale_1 * b_scale_1;
              }
              auto shifted_accum = final_accum + local_idx * WGMMA::kNumAccum;
              // loop by 4 since the assembly instruction groups values into 4s,
              // kNumAccum = N / 2

#pragma unroll
              for (uint32_t i = 0; i < WGMMA::kNumAccum / 4; i++) {
                const bool &predicate = kIsUniformBScales || i < num_former_iters;
              ...
              //   printf(" k block idx : %d, a_scale_0 : %f, a_scale_1 : %f, b_scale_1 : %f \n", k_block_idx,
              //   a_scale_0,
              //          a_scale_1, b_scale_0);
              //   printf("accum 0 : %f, accum 1 : %f", shifted_accum[0], shifted_accum[1]);
              // }
            }
          }
        });
...
#pragma unroll
      ...
      constexpr uint32_t TMA_D_BLOCK_N =
          kSwizzleDMode == 0 ? bN : (kSwizzleDMode / kNumElemBytes); // size in the N-direction of D swizzle atom
      constexpr uint32_t WGMMA_M_PER_WARP = WGMMA::M / 4;

      CUTE_STATIC_ASSERT(bM % 8 == 0, "Invalid bM size - not divisible by 8 so invalid swizzling atom for K-major");
      CUTE_STATIC_ASSERT(bN % TMA_D_BLOCK_N == 0 && bN / TMA_D_BLOCK_N <= 32,
                         "Unaligned TMA store (bN not aligned with atom size) or too many TMA store instructions");
      CUTE_STATIC_ASSERT(TMA_D_BLOCK_N % 8 == 0, "Invalid TMA Block N"); // need this for the 8 x 8
      ...
      // this check does nothing for sm90, since atom N size is always divisible by 4
      CUTE_STATIC_ASSERT(WGMMA::kNumAccum % 4 == 0, "Invalid STSM x2 vectorization");

#pragma unroll
      for (uint32_t local_idx = 0; local_idx < bM / WAVE_BLOCK_M; local_idx++) {
        auto m_offset = local_idx * WAVE_BLOCK_M;
        auto shifted_accum = final_accum + WGMMA::kNumAccum * local_idx;
        __nv_bfloat16 *shifted_smem_ptr =
            reinterpret_cast<__nv_bfloat16 *>(reinterpret_cast<uint8_t *>(sD) + m_offset * kSwizzleDMode);
        store_accum_to_swizzled_smem<bN, bM, kSwizzleDMode, WGMMA_M_PER_WARP>(shifted_accum, shifted_smem_ptr, warpIdx,
                                                                              lane_idx);
      }
      ...
      CUTE_STATIC_ASSERT(kNumWGMMAStoreThreads >= bN / TMA_D_BLOCK_N, "Too many TMA blocks");

      if (threadIdx.x < bN / TMA_D_BLOCK_N) {
        auto in_block_n_offset = threadIdx.x * TMA_D_BLOCK_N;
        auto smem_ptr = sD + in_block_n_offset * bM;
        if constexpr (kGemmType == GemmType::MGroupedMasked) {
          cute::SM90_TMA_STORE_3D::copy(&d_tensor_map, smem_ptr, n_block_idx * bN + in_block_n_offset,
                                        scheduler.template get_global_idx<false>(M, m_block_idx, bM),
                                        scheduler.cur_group_idx);
        } else {
          cute::SM90_TMA_STORE_2D::copy(&d_tensor_map, smem_ptr, n_block_idx * bN + in_block_n_offset,
                                        scheduler.template get_global_idx<false>(M, m_block_idx, bM));
        }
