...
inline cudaDataType_t tensorDType_to_cudaDType(c10::ScalarType type) {
  switch (type) {
  case c10::ScalarType::Float:
    return CUDA_R_32F;
  case c10::ScalarType::Half:
...
template <typename T> inline cudaDataType_t toCudaDType() {
...
// default trait
template <typename T> struct compute_t {
  using type = T;
};
// bfloat16 trait
template <> struct compute_t<__nv_bfloat16> {
  using type = float;
};
template <> struct compute_t<__half> {
  using type = float;
};
template <typename T> using compute_t_t = typename compute_t<T>::type;

// helper function for casting
template <typename T> inline __device__ compute_t_t<T> to_compute(T val) {
...
template <typename T> inline __device__ T from_compute(compute_t_t<T> val) {
  if constexpr (std::is_same<T, __nv_bfloat16>::value) {
...
template <> struct tDType_t<c10::ScalarType::Float> {
  using type = float;
};
template <> struct tDType_t<c10::ScalarType::BFloat16> {
  using type = __nv_bfloat16;
};
template <> struct tDType_t<c10::ScalarType::Half> {
  using type = __half;
};
