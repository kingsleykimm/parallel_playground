...
inline bool check_tensor_close(const torch::Tensor &tensor1,
                               const torch::Tensor &tensor2, float atol,
                               float rtol) {
  ...
  if (!pass) {
    ...
    printf("\033[0;31mTensor1 shape: %s, Strides: %s\033[0m\n",
           shape_to_string(tensor1.sizes().vec()).c_str(),
           shape_to_string(tensor1.strides().vec()).c_str());
    printf("\033[0;31mTensor2 shape: %s, Strides: %s\033[0m\n",
           shape_to_string(tensor2.sizes().vec()).c_str(),
           shape_to_string(tensor2.strides().vec()).c_str());
    std::cout << "\033[0;31mInd Diff: "
              << torch::abs(tensor1 - tensor2).argmax().item() << "\033[0m"
              << std::endl;
    std::cout << "\033[0;31mDiff: "
              << torch::abs(tensor1 - tensor2).max().item() << "\033[0m"
              << std::endl;
    std::cout << "\033[0;31mAvg Diff : "
              << torch::abs(tensor1 - tensor2).mean().item() << "\033[0m"
              << std::endl;
    std::cout << "\033[0;31mTensor 1 : " << tensor1.flatten()[argmax_ind].item()
              << "\033[0m" << std::endl;
    std::cout << "\033[0;31mTensor 2 : " << tensor2.flatten()[argmax_ind].item()
              << "\033[0m" << std::endl;
  } else {
    int argmax_ind = torch::abs(tensor1 - tensor2).argmax().item().toInt();
    ...
    printf("\033[0;32mTensor2 shape: %s, Strides: %s\033[0m\n",
           shape_to_string(tensor2.sizes().vec()).c_str(),
           shape_to_string(tensor2.strides().vec()).c_str());
    std::cout << "\033[0;32mInd Diff: "
              << torch::abs(tensor1 - tensor2).argmax().item().toFloat()
              << "\033[0m" << std::endl;
    std::cout << "\033[0;31mAvg Diff : "
              << torch::abs(tensor1 - tensor2).mean().item() << "\033[0m"
              << std::endl;
    std::cout << "\033[0;31mDiff: "
              << torch::abs(tensor1 - tensor2).max().item().toFloat()
              << "\033[0m" << std::endl;
    std::cout << "\033[0;32mTensor 1 : " << tensor1.flatten()[argmax_ind].item()
              << "\033[0m" << std::endl;
    std::cout << "\033[0;32mTensor 2 : " << tensor2.flatten()[argmax_ind].item()
              << "\033[0m" << std::endl;
  }
  return pass;
}
...
inline bool check_tensor_close(torch::Tensor &tensor1, at::Tensor &tensor2,
                               float atol, float rtol) {
  (void)sizeof(ScalarType);
  return check_tensor_close(static_cast<const torch::Tensor &>(tensor1),
                            static_cast<const torch::Tensor &>(tensor2), atol,
                            rtol);
}

// Quantize a 2D tensor to FP8 with 1D block scaling (per-row, 128-element
// blocks) Returns a tuple of (quantized_tensor, scale_factors) Scale factors
// have shape (num_rows, num_k_blocks) where num_k_blocks = K / 128
inline std::tuple<torch::Tensor, torch::Tensor>
quantize_fp8_1d_block(const torch::Tensor &t, Major sf_major,
  ...
  auto t_float = t.ndimension() == 2
                     ? t.unsqueeze(0).to(torch::kFloat32).contiguous().cpu()
                     : t.to(torch::kFloat32).contiguous().cpu();
  const auto shapes = t_float.sizes().vec();
  const int64_t k_dim = t_float.size(-1);
  HOST_ASSERT(k_dim % 128 == 0,
              "K dimension must be divisible by 128 for FP8 block scaling");

  int num_k_blocks = k_dim / 128;
  torch::TensorOptions sf_options =
  ...
  if (t.ndimension() == 2) {
    ...
    // quantized_fp8 = quantized_fp8.reshape({orig_sizes[0], orig_sizes[1],
    // orig_sizes[2]}); sf_device = sf_device.reshape({orig_sizes[0],
    // orig_sizes[1], num_k_blocks}).to(device);
  }
...
inline std::tuple<torch::Tensor, torch::Tensor>
quantize_fp8_2d_block(const torch::Tensor &t,
                      torch::Device device = torch::kCUDA) {
  ...
  HOST_ASSERT(k_dim % 128 == 0,
              "K dimension must be divisible by 128 for FP8 block scaling");
  HOST_ASSERT(n_dim % 128 == 0,
              "N dimension must be divisible by 128 for 2D FP8 block scaling");
...
inline double calc_diff(const torch::Tensor &ref, const torch::Tensor &out) {
  ...
  double similarity =
      (2 * (ref_d * out_d).sum().item().toDouble()) / denominator;
  return 1 - similarity;
}
...
// aren't assigned to any expert. -1 rows are placed at periodic intervals
// within each group section, never on block-aligned boundaries (to avoid TMA
// issues).
inline std::tuple<torch::Tensor, std::vector<size_t>, std::vector<size_t>>
generate_contiguous_grouped_layout(int &total_rows, int num_groups,
                                   int expected_m_per_group,
                                   torch::Device device = torch::kCUDA,
                                   bool padded = true, uint64_t seed = 1234) {
  ...
  torch::Tensor layout =
      torch::empty({total_rows}, torch::TensorOptions().dtype(torch::kInt32));
...
// Returns a tensor of shape (num_groups,) containing the number of valid rows
// per group
inline torch::Tensor generate_masked_grouped_layout(
    int64_t max_rows_per_group, int64_t expected_rows_per_group, int num_groups,
    torch::Device device = torch::kCUDA, uint64_t seed = 1234) {
  std::mt19937 gen(static_cast<std::mt19937::result_type>(seed));
  std::uniform_real_distribution<float> dist(0.7f, 1.3f);

  torch::Tensor layout =
      torch::empty({num_groups}, torch::TensorOptions().dtype(torch::kInt32));
  auto accessor = layout.accessor<int32_t, 1>();

  for (int g = 0; g < num_groups; g++) {
    int64_t rows = static_cast<int64_t>(dist(gen) * expected_rows_per_group);
...
inline torch::Tensor
generate_masked_grouped_layout(int64_t max_rows_per_group, int num_groups,
                               torch::Device device = torch::kCUDA) {
