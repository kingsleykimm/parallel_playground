/**
 * @brief Test for kernel3: grouped FP8 GEMM with fused SwiGLU + FP8
 * requantization.
 *
 ...
 *
 * Dequantize: D_fp32[m, cb*BN:(cb+1)*BN] = D_fp8[m, cb*BN:(cb+1)*BN] *
 * scale_d[cb, m]
 *
...
static torch::Tensor compute_swiglu_ref(
    ...
    int64_t total_M, int groups, int64_t N_out, torch::Device dev) {
  auto ref =
      torch::zeros({total_M, N_out},
...
static torch::Tensor
dequantize_fp8(const torch::Tensor &D_fp8,   // (total_M, N_out)
               const torch::Tensor &scale_d) // (cblocks, total_M)
{
  ...
  auto scale_b = scale_t.unsqueeze(-1)
                     .expand({total_M, cblocks, KERNEL3_BN})
...
static bool run_contiguous(int64_t expected_m_per_group, int64_t N, int64_t K,
  ...
  auto fp8_opts =
      torch::TensorOptions().dtype(torch::kFloat8_e4m3fn).device(dev);
  ...
  auto [layout_gpu, actual_ms, aligned_ms] =
      test_utils::generate_contiguous_grouped_layout(
          total_rows, groups, static_cast<int>(expected_m_per_group), dev,
          /*padded=*/true, seed);
  ...
  auto ref =
      compute_swiglu_ref(A, gate, up, layout_cpu, total_M, groups, N_out, dev);
  ...
  auto [gate_fp8_flat, sf_gate] =
      test_utils::quantize_fp8_2d_block(gate_flat, dev);
  ...
  if (!valid_rows.empty()) {
    ...
    if (verbose)
      test_utils::check_tensor_close(ref_v, out_v, 0.1f, 0.1f);
  ...
  std::cout << (diff < 0.005 ? "[PASSED] " : "[FAILED] ")
            << "contiguous M=" << expected_m_per_group << " N=" << N
            << " K=" << K << " G=" << groups << " total_M=" << total_M
            << " kernel_us=" << us << " diff=" << diff << "\n";
  return diff < 0.005;
}
...
static bool run_masked(int64_t max_M, int64_t expected_m_per_group, int64_t N,
  ...
  auto ref = compute_swiglu_ref(A_flat, gate, up, layout_flat, total_M, groups,
                                N_out, dev);

  // Quantize A: reshape to (total_M, K) for 1D block quant, then MN-major
  auto [A_fp8_flat, sfa] =
      test_utils::quantize_fp8_1d_block(A_flat, Major::K, dev);
  sfa = sfa.reshape({total_M, K / 128}).transpose(-1, -2).contiguous();
  ...
  if (!ref_parts.empty()) {
    ...
    if (verbose)
      test_utils::check_tensor_close(ref_cat, out_cat, 0.1f, 0.1f);
  ...
  auto us =
      std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();
  std::cout << (diff < 0.005 ? "[PASSED] " : "[FAILED] ")
            << "masked  max_M=" << max_M << " N=" << N << " K=" << K
            << " G=" << groups << " kernel_us=" << us << " diff=" << diff
            << "\n";
  return diff < 0.005;
}
...
int main(int argc, char **argv) {
  ...
  auto run_one = [&](const TestShape &s) {
    bool ok = (s.type == GemmType::MGroupedContiguous)
                  ? run_contiguous(s.M, s.N, s.K, s.groups, cfg.seed,
                                   cfg.verbose, cfg.is_consumer_pp)
                  : run_masked(s.M, s.expected_m_per_group, s.N, s.K, s.groups,
                               cfg.seed, cfg.verbose, cfg.is_consumer_pp);
    if (ok)
      ++passed;
    else
      ++failed;
