...
public:
    ...
    CUMemAllocHandle() : alloc_size_(0), device_id_(-1), handle_kind_(CUMemHandleKind::Local), handle_(0) {}

    CUMemAllocHandle(size_t size, int device_id, CUMemHandleKind handle_kind)
        : alloc_size_(0), device_id_(device_id), handle_kind_(handle_kind), handle_(0) {
    ...
    CUMemAllocHandle& operator=(CUMemAllocHandle&& other) noexcept {
        if (this != &other) {
            if (handle_) cuMemRelease(handle_);
            alloc_size_ = other.alloc_size_;
            device_id_ = other.device_id_;
    ...
    // Map this allocation into a virtual address, accessible from `access_device_id`
...
private:
    static CUmemAllocationHandleType_enum to_cu_handle_type(CUMemHandleKind kind) {
        switch (kind) {
            case CUMemHandleKind::Local:          return CU_MEM_HANDLE_TYPE_NONE;
            case CUMemHandleKind::FileDescriptor: return CU_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR;
            case CUMemHandleKind::Fabric:         return CU_MEM_HANDLE_TYPE_FABRIC;
        }
...
private:
    CUMemImportHandle(CUmemGenericAllocationHandle handle, size_t size, int device_id)
