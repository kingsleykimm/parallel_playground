...
auto with_optional_stream(int device, cudaStream_t stream, Fn&& fn) {
    if (stream != nullptr && device >= 0) {
        c10::cuda::CUDAStreamGuard guard(c10::cuda::getStreamFromExternal(stream, device));
        return fn();
    }
    return fn();
}

inline std::string to_string(const at::Tensor& t, int n, int s = 0, int e = -1) {
    (void)e;
    if (!t.defined()) {
    ...
    stream << "Tensor(shape=" << c10::str(t.sizes()) << ", dtype=" << type_to_string(dtype_of(t)) << ")\n";

    auto flat = t.flatten();
    const int64_t numel = flat.numel();
    const int64_t start = std::max<int64_t>(0, s);
    const int64_t count = std::max<int64_t>(0, std::min<int64_t>(n, numel - start));
...
template <typename T>
inline std::vector<T> tolist(const at::Tensor& t) {
...
template <typename T>
constexpr c10::ScalarType cpp_type_to_dtype() {
...
inline Tensor empty(const std::vector<size_t>& shape,
...
inline Tensor empty_like(const Tensor& self,
    ...
    auto out = with_optional_stream(to_int_device(self), stream, [&]() { return torch::empty_like(self, opts); });
...
inline Tensor zeros_like(const Tensor& self,
                         ...
                         bool blocking = false) {
    auto opts = dtype.has_value() ? self.options().dtype(*dtype) : self.options();
    auto out = with_optional_stream(to_int_device(self), stream, [&]() { return torch::zeros_like(self, opts); });
    if (blocking && stream != nullptr && to_int_device(self) >= 0) {
        CUDA_CHECK(cudaStreamSynchronize(stream));
    }
    return out;
...
inline Tensor ones_like(const Tensor& self,
                        ...
                        bool blocking = false) {
    auto opts = dtype.has_value() ? self.options().dtype(*dtype) : self.options();
    auto out = with_optional_stream(to_int_device(self), stream, [&]() { return torch::ones_like(self, opts); });
    if (blocking && stream != nullptr && to_int_device(self) >= 0) {
        CUDA_CHECK(cudaStreamSynchronize(stream));
    }
    return out;
...
Tensor full(const std::vector<size_t>& shape, T fill_value, int device) {
    ...
    return torch::full(to_i64_shape(shape), fill_value, make_options(dtype, device));
}

template <typename T>
Tensor full_like(const Tensor& self, T fill_value) {
    const auto dtype = cpp_type_to_dtype<T>();
    return torch::full(to_i64_shape(self.sizes()), fill_value, make_options(dtype, to_int_device(self)));
}
...
inline Tensor deepcopy(Tensor& self, cudaStream_t stream = nullptr, bool blocking = false) {
    auto out = with_optional_stream(to_int_device(self), stream, [&]() { return self.clone(); });
    if (blocking && stream != nullptr && to_int_device(self) >= 0) {
        CUDA_CHECK(cudaStreamSynchronize(stream));
    }
    return out;
...
inline Tensor shallow_copy(Tensor& self) {
    return self;
}
...
inline Tensor concat(const std::vector<Tensor*>& tensors, int dim, cudaStream_t stream = nullptr) {
    HOST_ASSERT(!tensors.empty(), "tensor_compat.h: concat requires non-empty tensor list");
    std::vector<Tensor> copied;
    copied.reserve(tensors.size());
    for (const auto* t : tensors) {
        HOST_ASSERT(t != nullptr, "tensor_compat.h: concat does not accept null tensors");
        copied.push_back(*t);
    }
    const int device = to_int_device(copied.front());
    return with_optional_stream(device, stream, [&]() { return torch::cat(copied, dim); });
}

inline Tensor concat(const std::vector<Tensor*>& tensors, int dim, StreamPool& streams) {
    cudaStream_t stream = nullptr;
...
inline Tensor concat(const std::vector<Tensor>& tensors, int dim, cudaStream_t stream = nullptr) {
    ...
    return with_optional_stream(device, stream, [&]() { return torch::cat(tensors, dim); });
}

inline Tensor stack(const std::vector<Tensor*>& tensors, int dim, cudaStream_t stream = nullptr) {
    HOST_ASSERT(!tensors.empty(), "tensor_compat.h: stack requires non-empty tensor list");
    std::vector<Tensor> copied;
    copied.reserve(tensors.size());
    for (const auto* t : tensors) {
        HOST_ASSERT(t != nullptr, "tensor_compat.h: stack does not accept null tensors");
        copied.push_back(*t);
    }
    const int device = to_int_device(copied.front());
    return with_optional_stream(device, stream, [&]() { return torch::stack(copied, dim); });
}

inline Tensor stack(const std::vector<Tensor*>& tensors, int dim, StreamPool& streams) {
    cudaStream_t stream = nullptr;
    int stream_id = streams.fetchStream(stream);
...
inline Tensor repeat(Tensor& self,
                     int dim,
                     size_t num_repeats,
                     cudaStream_t stream = nullptr,
                     bool blocking = false) {
    ...
    HOST_ASSERT(dim >= 0 && dim < ndim, "tensor_compat.h: repeat dim out of bounds");
    std::vector<int64_t> repeats(static_cast<size_t>(ndim), 1);
    repeats[dim] = static_cast<int64_t>(num_repeats);
    auto out = with_optional_stream(to_int_device(self), stream, [&]() { return self.repeat(repeats); });
    if (blocking && stream != nullptr && to_int_device(self) >= 0) {
        CUDA_CHECK(cudaStreamSynchronize(stream));
    }
    return out;
...
inline Tensor repeat_interleave(Tensor& self,
                                int dim,
                                size_t num_repeats,
                                cudaStream_t stream = nullptr,
                                bool blocking = false) {
    ...
    HOST_ASSERT(dim >= 0 && dim < ndim, "tensor_compat.h: repeat_interleave dim out of bounds");
    auto out = with_optional_stream(to_int_device(self), stream, [&]() {
        return torch::repeat_interleave(self, static_cast<int64_t>(num_repeats), dim);
    });
    if (blocking && stream != nullptr && to_int_device(self) >= 0) {
        CUDA_CHECK(cudaStreamSynchronize(stream));
    }
...
inline Tensor squeeze(Tensor other) {
    return other.squeeze();
}

inline void unsqueeze(Tensor& other, int dim) {
    other = other.unsqueeze(dim);
}
...
template <typename T>
Tensor pad_sequence(std::vector<Tensor*> sequences,
                    bool batch_first,
                    T padding_value,
                    std::string padding_side,
                    StreamPool& streams) {
    (void)streams;
    HOST_ASSERT(!sequences.empty(), "tensor_compat.h: pad_sequence expects non-empty input");

    std::vector<Tensor> seq;
    seq.reserve(sequences.size());
    for (auto* t : sequences) {
        HOST_ASSERT(t != nullptr, "tensor_compat.h: pad_sequence got a null tensor");
        HOST_ASSERT(t->dim() <= 1, "tensor_compat.h: pad_sequence supports 1D tensors only");
        seq.push_back(*t);
    }
    ...
    HOST_ASSERT(padding_side == "left", "tensor_compat.h: padding_side must be left or right");
    int64_t max_len = 0;
    ...
    for (size_t i = 0; i < seq.size(); ++i) {
        ...
        if (batch_first) {
            ...
            out.narrow(1, static_cast<int64_t>(i), 1).squeeze(1).narrow(0, start, len).copy_(seq[i]);
        }
    }
    return out;
}
...
// Transitional global forwards for legacy call sites that used unqualified helpers.
inline c10::Device to_c10_device(int dev) {
    return custom::to_c10_device(dev);
}
...
inline bool is_mn_major(const at::Tensor& t) {
    return custom::is_mn_major(t);
}

inline Major major_of(const at::Tensor& t) {
    return custom::major_of(t);
}
