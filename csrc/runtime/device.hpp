...
public:
  ...
  size_t get_smem_size() { return get_prop()->sharedMemPerBlockOptin; }

  size_t get_sm_smem_size() { return get_prop()->sharedMemPerMultiprocessor; }
  ...
  std::pair<size_t, size_t> get_max_clusters() {
    ...
    if (major >= 9) {
      ...
      HOST_ERROR(
          "get_max_clusters: Cluster launch is not supported on this device");
    }
...
struct CublasHolder {
  ...
  std::shared_ptr<cublasLtHandle_t> get_handle() {
    if (handle_ptr == nullptr || cublas_workspace == nullptr) {
      ...
      CUBLAS_CHECK(
          cublasLtHeuristicsCacheSetCapacity(heuristics_cache_capacity));
...
static auto device_prop =
    LazyInit<DeviceProps>([]() -> std::shared_ptr<DeviceProps> {
      return std::make_shared<DeviceProps>();
    });
static auto cublas_holder =
    LazyInit<CublasHolder>([]() -> std::shared_ptr<CublasHolder> {
