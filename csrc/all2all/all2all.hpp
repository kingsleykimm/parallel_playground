...
public:
  All2All(uint32_t max_num_tokens, uint32_t num_experts,
          ...
          c10::ScalarType out_dtype, std::optional<c10::ScalarType> scale_dtype,
          uint32_t num_experts_per_token,
          std::optional<uint32_t> max_private_tokens_opt,
          std::optional<ParallelGroup> dp_group, ParallelGroup node_group,
          int device, ParallelGroup global_group, cudaStream_t stream) {
    ...
    this->num_local_experts =
        host_ceil_div(this->num_experts, this->world_size);

    // recv buffer size
    uint32_t avg_tokens_per_expert =
        host_ceil_div(max_num_tokens * num_experts_per_token, num_experts) *
        1.2;
    ...
    HOST_ASSERT(max_private_tokens > 0,
                "max_private_tokens must be greater than 0");

    uint32_t num_tokens = max_num_tokens * num_dp_groups;
    uint32_t max_recv_tokens =
        max_private_tokens *
        num_dp_groups; // private size for direct NVlink copies

    // first min is to ensure there are at most num_tokens * num_local_experts,
    // the maximum possible upper bound where all tokens get mapped to this rank
    // the next max is to ensure at least num_local_experts * expert_padding is
    // on each rank - this is the lower bound of expert tokens mapped to this
    // device
    max_recv_tokens += host_align(
        std::max(std::min(num_tokens * num_experts_per_token +
    ...
    this->_num_routed_handle =
        CUMemAllocHandle(num_dp_groups * num_experts * sizeof(uint32_t), device,
    ...
    uint32_t token_dim_dispatch =
        host_align(hidden_dim * get_type_size(in_dtype), 16) + 16;
    if (hidden_dim_scale.has_value() && scale_dtype.has_value()) {

      token_dim_dispatch += host_align(
          hidden_dim_scale.value() * get_type_size(scale_dtype.value()), 16);

      HOST_ASSERT(scale_dtype.value() == c10::ScalarType::Float,
                  "Only float scales supported");
    }

    // combine token dim is just the hidden dim, since the outputs of MoE
    // kernels are unquantized

    uint32_t token_dim_combine =
        host_align(hidden_dim * get_type_size(in_dtype), 16);
    uint32_t token_dim = std::max(token_dim_combine, token_dim_dispatch);

    uint32_t send_buffer_bytes =
        host_align(max_recv_tokens * token_dim, PAGE_SIZE);
    this->_send_buffer_handle = CUMemAllocHandle(
        send_buffer_bytes, device, CUMemHandleKind::FileDescriptor);
    this->_send_buffer_mapping = _send_buffer_handle.map(this->device);

    this->_recv_buffer_handle = CUMemAllocHandle(
        send_buffer_bytes, device, CUMemHandleKind::FileDescriptor);
    ...
    if (node_group.size > 1) { // nvlink buffers
      ...
      this->_sync_buffer_handle =
          CUMemAllocHandle(node_group.size * 2 * sizeof(uint32_t), this->device,
                           CUMemHandleKind::FileDescriptor);
      CUMemMapping sync_mapping = _sync_buffer_handle.map(this->device);
      CUDA_CHECK(cudaMemset(sync_mapping.data_ptr(), 0,
                            sizeof(uint32_t) * node_group.size * 2));

      {
        auto local_handle =
            NVLinkLoad{this->_sync_buffer_handle.export_handle(),
        ...
        MPI_Allgather(&local_handle, sizeof(local_handle), MPI_BYTE,
                      handles.data(), sizeof(local_handle) * node_group.size,
                      MPI_BYTE, node_group.comm);
        ...
        for (size_t i = 0; i < handles.size(); i++) {
          ...
          if (i == this->node_group.rank) {
            this->nvl_rank_mappings.push_back(NVLRankMappings{
                std::move(sync_mapping), std::move(this->_recv_buffer_mapping),
                std::move(this->_send_buffer_mapping),
                std::move(this->_num_routed_mapping)});
          } else {
            this->nvl_rank_mappings.push_back(NVLRankMappings{
                CUMemImportHandle::from_export(handle.sync_fd)
                    .map(this->device),
                CUMemImportHandle::from_export(handle.recv_fd)
                    .map(this->device),
                CUMemImportHandle::from_export(handle.send_fd)
                    .map(this->device),
                CUMemImportHandle::from_export(handle.num_routed_fd)
                    .map(this->device)});
          }
        }
      ...
      for (uint32_t i = 0; i < node_group.size; i++) {
        recv_ptrs.push_back(
            (uint8_t *)this->nvl_rank_mappings[i].recv_mapping.data_ptr());
        send_ptrs.push_back(
            (uint8_t *)this->nvl_rank_mappings[i].recv_mapping.data_ptr());
        sync_ptrs.push_back(
            (uint32_t *)this->nvl_rank_mappings[i].recv_mapping.data_ptr());
        num_routed_ptrs.push_back((uint32_t *)this->nvl_rank_mappings[i]
                                      .num_routed_mapping.data_ptr());
      }
    }
    this->context = All2AllContext(
        hidden_dim, hidden_dim_scale, get_type_size(in_dtype),
        get_type_size(out_dtype), out_dtype,
        scale_dtype.has_value() ? get_type_size(scale_dtype.value()) : 0,
        max_num_tokens, max_recv_tokens, max_private_tokens, num_experts,
        expert_padding, num_experts_per_token, rank, dp_size, node_group.size,
        world_size, (uint32_t *)this->_num_routed_mapping.data_ptr(),
        (uint8_t *)this->_send_buffer_mapping.data_ptr(),
        (uint8_t *)this->_recv_buffer_mapping.data_ptr(), sync_ptrs.data(),
        send_ptrs.data(), recv_ptrs.data(), num_routed_ptrs.data(), device,
        stream);
  }
  ...
  void dispatch(
    ...
    uint32_t out_x_stride =
        get_type_size(this->in_dtype) * out_expert_x.stride(0); // in bytes
    ...
    if (out_expert_x_scale.has_value()) {
      HOST_ASSERT(out_expert_x_scale->dim() == 2,
                  "Expert x scale ndimensions = 2");
      HOST_ASSERT(
          this->scale_dtype.has_value(),
          "scale_dtype must be set when out_expert_x_scale is provided");
      HOST_ASSERT(dtype_of(*out_expert_x_scale) == this->scale_dtype.value(),
    ...
    if (dp_x_scale.has_value()) {
      ...
      HOST_ASSERT(this->scale_dtype.has_value(),
                  "scale_dtype must be set when dp_x_scale is provided");
      HOST_ASSERT(dtype_of(*dp_x_scale) == this->scale_dtype.value(),
                  "Dtype check");
    ...
    HOST_ASSERT(indices.dim() == 2 && indices.size(0) == dp_x.size(0) &&
                    indices.size(1) == this->num_experts_per_token,
                "indices shape check");
    ...
    HOST_ASSERT(weights.dim() == 2 && weights.size(0) == dp_x.size(0) &&
                    weights.size(1) == this->num_experts_per_token,
    ...
    if (do_send) {
      this->context.dispatch_send(dp_x.size(0), x_ptr, x_stride,
                                  x_scale_ptr,         // can be nullptr if not
                                  x_scale_stride_elem, // these can be 0
                                  x_scale_stride_token, indices_ptr,
                                  indices_stride, weights_ptr, weights_stride,
                                  bound_m_ptr, stream);
    }

    this->context.worker
        .process_routing_info(); // although there are cuda async functions in
                                 // here, they are all done on the same stream
                                 // per device

    if (do_recv) {
      this->context.dispatch_recv(
          out_expert_num_tokens_ptr, out_x_ptr, out_x_stride,
          reinterpret_cast<uint8_t *>(out_x_scale_ptr), out_x_scale_stride_elem,
          out_x_scale_stride_token, stream);
    }
  }
  ...
  void combine(at::Tensor &out_tokens, // (activations for this device)
    ...
    HOST_ASSERT(expert_y.dim() == 2,
                "outputs should be 2 dimensional (even for batched)");
    uint8_t *expert_y_ptr = expert_y.data_ptr<uint8_t>();
    uint32_t expert_y_stride =
        expert_y.stride(0) * get_type_size(dtype_of(expert_y));
    ...
    if (do_send) {
      this->context.combine_send(
          ...
          expert_y_stride, stream);
    }

    if (do_recv) {
      this->context.combine_recv(num_tokens, //
                                 num_recv_tokens, dtype_of(expert_y),
                                 out_tokens_ptr, out_tokens_stride, indices_ptr,
                                 indices_stride, weights_ptr, weights_stride,
                                 bound_m_ptr, accumulate, stream);
    }
  }
  ...
  //     cudaStream_t stream = nullptr
  // ) {
  //     all2all.dispatch(out_expert_num_tokens, out_expert_x,
  //     out_expert_x_scale,
...
private:
  ...
  bool initialized;
  CUMemAllocHandle _send_buffer_handle, _recv_buffer_handle,
      _sync_buffer_handle, _num_routed_handle;
  CUMemMapping _send_buffer_mapping, _recv_buffer_mapping, _num_routed_mapping;

  std::vector<NVLRankMappings>
      nvl_rank_mappings; // this is used to persist the mappings we receive from
                         // other devices after importing
...
class All2All {
public:
  template <typename... Args> explicit All2All(Args &&...) {
    HOST_ERROR("All2All is unavailable in this build");
  }

  void dispatch(at::Tensor &, at::Tensor &, std::optional<at::Tensor> &,
                at::Tensor &, std::optional<at::Tensor> &, at::Tensor &,
                at::Tensor &, std::optional<at::Tensor> &, bool = true,
                bool = true, cudaStream_t = nullptr) {
    HOST_ERROR("All2All dispatch is unavailable in this build");
  }

  void combine(at::Tensor &, at::Tensor &, at::Tensor &, at::Tensor &,
               std::optional<at::Tensor> &, bool = true, bool = true,
               bool = false, cudaStream_t = nullptr) {
