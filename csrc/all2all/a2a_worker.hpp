...
class WorkerState {
    ...
    WorkerState(
        ...
        uint32_t world_size,
        uint32_t device,
        uint32_t * num_routed_ptr, // on device, shape (num_dp_groups, num_experts)
        uint8_t * send_buffer_ptr,
        uint8_t * recv_buffer_ptr,
        uint32_t ** num_routed_ptrs,
        cudaStream_t stream
        ...
        this->device = device;
        this->num_routed_ptr = num_routed_ptr;
        this->send_buffer_ptr = send_buffer_ptr;
        this->recv_buffer_ptr = recv_buffer_ptr;
        ...
        this->route_write_op = [&] () {
            for (uint32_t i = 0; i < world_size; i++) {
                uint32_t peer_rank = (rank + 1) % world_size;

                uint32_t peer_group = peer_rank / dp_size;
                if (peer_group == dp_group || peer_rank % dp_size != dp_rank || peer_rank == rank) {
                    continue;
                }
                // this is the current num routed to copy to
                CUDA_CHECK(cuMemcpyAsync((CUdeviceptr) this->num_routed_ptrs[peer_rank], (CUdeviceptr) this->num_routed_ptr, num_experts * sizeof(uint32_t), stream));
            }
    ...
    void process_routing_info() {
        ...
        uint32_t last_local_expert = std::min(
            first_local_expert + experts_per_rank, this->num_experts
        );
        ...
        uint32_t num_recv_tokens = 0; // tracker of receiving tokens and offsets
        {
            ...
            for (uint32_t dp_group = 0; dp_group < num_dp_groups; dp_group++) {
                ...
                src_group_offset[dp_group] = num_recv_tokens; // cumulative sum of tokens_from_group
            ...
            for (uint32_t expert = first_local_expert; expert < last_local_expert; expert++) {
                ...
                for (uint32_t i = 0; i < routed; i++) {
                    if (peer_rank == this->rank) { 
                        ...
                        uint32_t index_on_rank = src_dispatch_count[peer_group]; // starts at 0 offset
                    ...
                    padded_index[last] = padded_offsets[local_expert] + expert_count[local_expert];
