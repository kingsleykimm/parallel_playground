...
// entrypoint - api methods should call this method. this is only instantiated
// once per proces at the start
...
public:
  ...
  All2All(uint32_t max_num_tokens, uint32_t num_experts,
    ...
    this->num_local_experts_ =
        host_ceil_div(this->num_experts_, this->world_size_);
    ...
    // after the recv buffer size, the is bounded between
    // [this->num_local_experts_ * expert_padding, num_tokens *
    // num_experts_per_token] the actual value is the
    this->max_recv_tokens_ = host_align(
    ...
    this->context_.emplace(
        ...
        scale_dtype.has_value()
            ...
            : std::nullopt,
        max_num_tokens, max_recv_tokens_, max_private_tokens, num_experts,
        expert_padding, num_experts_per_token, this->rank_, this->dp_size_,
  ...
  RoutingDebugState debug_routing_state(cudaStream_t stream = nullptr) const {
    ...
    HOST_ASSERT(this->context_.has_value(),
                "All2All context is not initialized");
    ...
    // CUDA_CHECK(cudaPointerGetAttributes(&attr,
    // this->context->worker.tokens_per_expert)); printf("tokens_per_expert is
    // in %s memory\n", attr.type == cudaMemoryTypeDevice ? "device" : "host");

    CUDA_CHECK(cudaMemcpyAsync((uint32_t *)state.tokens_per_expert.data_ptr(),
                               this->context_->worker.tokens_per_expert,
                               this->num_local_experts_ * sizeof(uint32_t),
                               cudaMemcpyDeviceToDevice, effective_stream));
    ...
    if (state.num_recv_tokens > 0) {
      CUDA_CHECK(cudaMemcpyAsync((uint32_t *)state.source_rank.data_ptr(),
                                 this->context_->worker.source_rank,
                                 state.num_recv_tokens * sizeof(uint32_t),
                                 cudaMemcpyDeviceToDevice, effective_stream));
      CUDA_CHECK(
          cudaMemcpyAsync((uint32_t *)state.source_dispatch_offset.data_ptr(),
                          this->context_->worker.source_dispatch_offset,
                          state.num_recv_tokens * sizeof(uint32_t),
                          cudaMemcpyDeviceToDevice, effective_stream));
      CUDA_CHECK(
          cudaMemcpyAsync((uint32_t *)state.combine_send_offset.data_ptr(),
                          this->context_->worker.combine_send_offset,
                          state.num_recv_tokens * sizeof(uint32_t),
                          cudaMemcpyDeviceToDevice, effective_stream));
      CUDA_CHECK(cudaMemcpyAsync((uint32_t *)state.padded_index.data_ptr(),
  ...
  at::Tensor debug_num_routed() const {
    ...
    std::memcpy(t.data_ptr<uint32_t>(), hw.data(),
  ...
  void dispatch(
    ...
    uint32_t out_x_stride =
        get_type_size(this->in_dtype_) * out_expert_x.stride(0); // in bytes
    ...
    // wait on dispatch_route_done flag, then scatter num_routed through
    // route_write_op

    std::atomic_ref<uint8_t> dispatch_route_ptr(
        *this->context_->worker.dispatch_route_done);
    ...
    CUDA_CHECK(cudaMemcpyAsync(out_expert_num_tokens_ptr,
                               this->context_->worker.tokens_per_expert,
                               this->num_local_experts_ * sizeof(uint32_t),
  ...
  void combine(at::Tensor &out_tokens, // (activations for this device)
    ...
    if (do_recv) {
      ...
      this->context_->combine_recv(out_tokens, indices, weights, accumulate,
                                   stream);
    }
  }
  ...
  const All2AllContext<EXPERTS_PER_TOKEN, NUM_EXPERTS, TOKEN_DIM> &
  context() const {
    HOST_ASSERT(context_.has_value(), "All2All context is not initialized");
    return context_.value();
  }
...
private:
  ...
  std::optional<All2AllContext<EXPERTS_PER_TOKEN, NUM_EXPERTS, TOKEN_DIM>>
      context_;
