...
struct SM90Arch {
  static constexpr uint32_t kMaxSharedMemoryPerBlock =
      232448; // 232 KB max shared memory per block
  static constexpr uint32_t kMaxSharedMemoryPerSM =
      232448;                              // 232 KB max shared memory per SM
  ...
  static std::vector<int> get_block_n_candidates(uint32_t &n, Major major) {
    // the n direction has more flexibility - the range of values is from 8 -
    // 256, but there's almost no reason to begin at 8
    std::vector<int> candidates;
    for (uint32_t block_n : {32, 64, 96, 128, 160, 192, 224,
                             256}) { // for TK only up to BN = 32 is supported
  ...
  static std::pair<int, int> get_num_threads(uint32_t &block_m) {
    return std::make_pair(128,
                          (block_m <= 64 ? 1 : 2) *
                              128); // returns (numTMAThreads, numWGMMAThreads)
  }

  static bool is_block_legal(Major major_a, Major major_b,
                             c10::ScalarType AB_type, c10::ScalarType CD_type,
                             const uint32_t &block_m, const uint32_t &block_n,
                             const uint32_t &block_k, const uint32_t &m,
                             const uint32_t &n, const uint32_t &k) {

    // avoid too many scaling factors in a single block:
    // when BLOCK_N > BLOCK_K, gcd(BLOCK_N, BLOCK_K) = BLOCK_N - BLOCK_K
    // for kernel type Kernel::1D2D, when block_n > 128, the only values that
    // work are 128 + 16, 128 + 32, 128 + 64
    if (block_n > 128 && (block_n != 144 && block_n != 160 && block_n != 192))
      return false;
  ...
  static bool is_num_stages_legal(c10::ScalarType &AB_type,
    ...
    // first, we want to check if the compiler is going to unroll the
    // dispatch_num_former_iters when it does we check that num_stages <= 4 is
    // to manage code size
    if (search_space <= 4 && block_k % block_n != 0 &&
        AB_type == c10::ScalarType::Float8_e4m3fn) {
      return num_stages <= 4;
    }
    return true;
  }
  ...
  get_multicast_legality(const GemmType &gemm_type, const uint32_t &num_groups,
    ...
    return {// to check if multicast is legal on A, where we are broadcasting A
            // across N dimension
            // When performing masked grouped GEMM, we need the N blocks to be
            // even to avoid weight mixing
            is_multicast_legal(n, block_n, 2, num_sms,
                               gemm_type == GemmType::MGroupedMasked),
            // when multicasting B, for masked grouped gemms we always ensure
            // that the grid size % 2 == 0.
            // this is mGroupedMasked defines a variable size grid, instead of
            // the fixed (bM, bN), since it stores variable chunks of different
            // group lengths
  ...
  // simple for now - in the future it can be changed with sharding across
  // cluster dim
  static int get_a_load_m(const uint32_t &block_m) { return block_m; }

  static int get_b_load_n(const uint32_t &block_n) { return block_n; }

  static int get_sf_smem_size(const uint32_t &block_m, const uint32_t &block_n,
                              const uint32_t &block_k) {
  ...
  static int get_cd_store_m(const bool is_single_warpgroup,
  ...
  static int get_smem_cd_size(const uint32_t &block_m, const uint32_t &block_n,
    ...
    // for TMA / GMMA swizzling : max swizzle mode is 128, with 8 row / column
    // atoms
    return host_align(block_m * block_n * get_type_size(cd_type), 1024);
  }
};
