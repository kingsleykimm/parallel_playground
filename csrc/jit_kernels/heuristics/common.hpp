...
inline at::Tensor transform_sf_layout_mn(at::Tensor& sf_tensor, cudaStream_t & stream) {
    ...
    if (major_of(sf_tensor) != Major::MN) {
        ...
        at::Tensor transposed = custom::empty({num_groups, tma_aligned_mn, k}, dtype_of(sf_tensor), to_int_device(sf_tensor), stream);
