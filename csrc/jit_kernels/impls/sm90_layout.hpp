    ...
    public:
        ...
        static void launch_impl(KernelHandle& kernel, const LaunchConfigHandle& launch_config, const Args& args) {
            CUDA_CHECK(launch_kernel(kernel, launch_config,
                args.sf, args.out, args.mn, args.aligned_mn));
        }
};
...
inline void sm90_transpose_sf(at::Tensor& sf, at::Tensor& transposed_sf, cudaStream_t& stream) {
    // determine num_threads ,block_mn, sf_k
    // HOST_ASSERT(sf.dim() < 3, "scale factor ndim should be less than 2 for GEMMs");
    HOST_ASSERT(sf.dim() == 2 || sf.dim() == 3, "scale factor ndim should be 2 or 3 for GEMMs");
    ...
    SM90_Transpose_SF_Runtime::Args args = {
    ...
    std::shared_ptr<KernelRuntime> runtime = compiler->build("sm90_transpose_sf", code);
    LaunchRuntime<SM90_Transpose_SF_Runtime>::launch(runtime, args);
}

inline void sm90_transpose_bf16(
    __nv_bfloat16 * input,
...
)
{
    ...
    SM90_Transpose_BF16_Runtime::Args args = {
        static_cast<uint32_t>(num_threads),
        (uint32_t)block_mn ,
        static_cast<uint32_t>(k),
        input,
...
inline void sm90_transpose_fp32(
    float * input,
    float * output,
    size_t mn,
    size_t k,
