...
// JIT runtime for kernel4::global_kernel4 (grouped FP8 GEMM with ping-pong
// consumers) GEMM_TYPE: 0 = MGroupedMasked, 1 = MGroupedContiguous
class Kernel4Runtime : LaunchRuntime<Kernel4Runtime> {
public:
  struct Args {
    uint32_t M, N, K;
    uint32_t num_groups;
    void *A, *gate_weight, *up_weight, *D, *scale_a, *scale_gate, *scale_up,
        *scale_d;
...
>);
  ...
  static void launch_impl(KernelHandle &kernel,
    ...
    size_t gsize =
        tk_kernel4_globals_size(args.bn, args.bk, args.gemm_type, args.c_dtype);
    alignas(128) char globals_buf[2048];
    assert(gsize <= sizeof(globals_buf));
    tk_build_kernel4_globals(
        args.bn, args.bk, args.gemm_type, static_cast<int>(args.num_groups),
        args.c_dtype, globals_buf, args.A, args.gate_weight, args.up_weight,
        args.D, args.scale_a, args.scale_gate, args.scale_up, args.scale_d,
...
//   A:             (total_M, K)              — tokens from all groups
//   concatenated gate_weight:   (num_groups, N, K)        — gate projection
//   weights up_weight:     (num_groups, N, K)        — up   projection weights
//   scale_a:       (K/128, total_M)          — per-token K-block scales
//   (MN-major) scale_gate:    (num_groups * N/128, K/128) scale_up: (num_groups
//   * N/128, K/128) grouped_layout: (total_M,) int32         — group index per
...
inline void kernel4_contiguous(
  ...
  auto gemm_config =
      get_kernel3_config(GemmType::MGroupedContiguous, total_M, N, K, 1,
                         Major::K, Major::K, Major::K, A.scalar_type(),
                         D.scalar_type(), device_prop->get_num_sms(), true);
  ...
  LaunchConfig launch_config = {
      ...
      gemm_config.smem_config.smem_size, 1};
  ...
  if (get_env<int>("JIT_DEBUG") > 0) {
    ...
    printf("  bm=%d bn=%d bk=%d super_n=%d stages=%d\n", gemm_config.block_m,
           gemm_config.block_n, gemm_config.block_k, super_n,
  ...
  std::shared_ptr<KernelRuntime> runtime =
      compiler->build("kernel4_contiguous", code);
...
inline void
kernel4_masked(torch::stable::Tensor &A, torch::stable::Tensor &up_weight,
  ...
  auto gemm_config =
      get_kernel3_config(GemmType::MGroupedMasked, max_M, N, K, num_groups,
                         Major::K, Major::K, Major::K, A.scalar_type(),
                         D.scalar_type(), device_prop->get_num_sms(), true);
  ...
  const std::string &code = LaunchRuntime<Kernel4Runtime>::generate(args);
  std::shared_ptr<KernelRuntime> runtime =
      compiler->build("kernel4_masked", code);
  LaunchRuntime<Kernel4Runtime>::launch(runtime, args);
}
