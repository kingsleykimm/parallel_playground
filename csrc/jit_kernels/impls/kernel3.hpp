...
public:
    struct Args {
        uint32_t M, N, K; // 
        uint32_t num_groups;
        void *A, *up_weight, *gate_weight, *D, *scale_a, *scale_up, *scale_gate, *scale_d;
...
static void __instantiate_kernel() {{
    ...
    static void launch_impl(KernelHandle& kernel, const LaunchConfigHandle& launch_config, const Args& args) {
        ...
        size_t gsize = tk_kernel3_globals_size(args.bm, args.bn, args.bk, args.gemm_type, args.c_dtype);
        alignas(128) char globals_buf[2048];
        assert(gsize <= sizeof(globals_buf));
        tk_build_kernel3_globals(
...
//   D:             (total_M, N/2)         — FP8 output (silu(gate) * up, requantized)
//   scale_d:       (N/(2*BN), total_M)    — per-(row, col-block) output scale
    ...
    at::Tensor& D, int* grouped_layout, cudaStream_t& stream)
{
    ...
    const Kernel3Runtime::Args args = {
        ...
        .A = A.data_ptr(), .up_weight = up_weight.data_ptr(), .gate_weight = gate_weight.data_ptr(), .D = D.data_ptr(),
        .scale_a = scale_a.data_ptr(), .scale_up = scale_up.data_ptr(), .scale_gate = scale_gate.data_ptr(), .scale_d = scale_d.data_ptr(),
    ...
    std::shared_ptr<KernelRuntime> runtime = compiler->build("kernel3_contiguous", code);
...
//   D:             (num_groups * max_M, N/2) — FP8 output
//   scale_d:       (N/(2*BN), num_groups * max_M) — per-(row, col-block) output scale
inline void kernel3_masked(
    at::Tensor& A, at::Tensor& up_weight, at::Tensor& gate_weight,
    at::Tensor& scale_a, at::Tensor& scale_up, at::Tensor& scale_gate, at::Tensor& scale_d,
    at::Tensor& D, int* grouped_layout, cudaStream_t& stream)
{
    ...
    const std::string& code = LaunchRuntime<Kernel3Runtime>::generate(args);
    std::shared_ptr<KernelRuntime> runtime = compiler->build("kernel3_masked", code);
    LaunchRuntime<Kernel3Runtime>::launch(runtime, args);
}
