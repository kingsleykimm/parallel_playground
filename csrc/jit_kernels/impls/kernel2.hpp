...
inline void assert_cuda_device_ptr(const void *ptr, const char *name) {
  ...
  HOST_ASSERT(
      err == cudaSuccess,
      fmt::format("{} is not a CUDA pointer or cudaPointerGetAttributes failed",
                  name)
          .c_str());
  ...
  if (attr.type == cudaMemoryTypeDevice) {
    HOST_ASSERT(attr.device == current_device,
                fmt::format("{} is on device {} but current device is {}", name,
...
inline void assert_grouped_gemm_ptrs_device_resident(
    const void *A, const void *B, const void *C, const void *scale_a,
    const void *scale_b, const void *grouped_layout) {
...
inline std::string kernel2_trace_define_block() {
  return fmt::format("#define KERNEL2_TRACE_STAGE {}\n"
...
class SM90_FP8_GroupedGEMM_Runtime
    : LaunchRuntime<SM90_FP8_GroupedGEMM_Runtime> {
public:
  struct Args {
    uint32_t M, N, K;
    uint32_t num_groups;
...
static void __instantiate_kernel() {{
    auto ptr = reinterpret_cast<void*>(
        &kittens::prototype::lcf::kernel<mmt_jit>);
                       ...
                       kernel2_trace_define_block(), args.M, args.N, args.K,
                       args.bm, args.bn, args.bk, args.num_groups,
                       args.num_consumer_warps, args.num_producer_warps,
                       args.num_stages, args.smem_size, args.gemm_type,
                       to_string(args.c_dtype), args.super_n);
  }
  ...
  static void launch_impl(KernelHandle &kernel,
    ...
    size_t gsize = tk_grouped_globals_size(args.bm, args.bn, args.bk,
                                           args.gemm_type, args.c_dtype);
    alignas(128) char globals_buf[2048];
    assert(gsize <= sizeof(globals_buf));
    tk_build_grouped_globals(args.bm, args.bn, args.bk, args.gemm_type,
                             static_cast<int>(args.num_groups), args.c_dtype,
                             globals_buf, args.A, args.B, args.C, args.scale_a,
...
inline void sm90_fp8_grouped_gemm_contiguous(
  ...
  const std::string &code =
      LaunchRuntime<SM90_FP8_GroupedGEMM_Runtime>::generate(args);
  std::shared_ptr<KernelRuntime> runtime =
      compiler->build("sm90_fp8_grouped_gemm_contiguous", code);
...
inline void sm90_fp8_grouped_gemm_masked(
  ...
  const std::string &code =
      LaunchRuntime<SM90_FP8_GroupedGEMM_Runtime>::generate(args);
  std::shared_ptr<KernelRuntime> runtime =
      compiler->build("sm90_fp8_grouped_gemm_masked", code);
  LaunchRuntime<SM90_FP8_GroupedGEMM_Runtime>::launch(runtime, args);
}
