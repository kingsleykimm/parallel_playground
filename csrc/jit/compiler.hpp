...
static std::string get_gcc_system_include_flags() {
  auto [exit_code, output] =
      run_command("gcc -v -x c++ /dev/null -fsyntax-only 2>&1");
  ...
  while (std::getline(ss, line)) {
    ...
    if (in_search) {
      ...
      if (path.rfind("/usr", 0) != 0 && path.rfind("/lib", 0) != 0)
        continue;
...
public:
  ...
  static std::string get_library_version() {
    ...
    for (const auto &entry :
         all_files_in_dir(library_include_path / "moe_cuda")) {
  ...
  static void init_static_vars(fs::path library_root_path, fs::path cuda_home) {
    Compiler::library_root_path = library_root_path;
    Compiler::cuda_home = cuda_home;
    KernelRuntime::cuda_home = cuda_home;
    const auto include_override =
        get_env<std::string>("LIBRARY_INCLUDE_PATH", "");
  ...
  Compiler() {
    ...
    signature = "unknown";
    flags = fmt::format("-std=c++{} --diag-suppress=177 "
  ...
  /*
  Given the current name and code from the runtime, this method searches the jit
  cache to see if the KernelRuntime object already exists. Otherwise it compiles
  with NVCC and then adds into the cache
  */
  ...
  void write_file(const fs::path &file_name, const std::string &data) const {
    ...
    HOST_ASSERT(out.is_open(),
                fmt::format("Failed to open temporary file for writing: {}",
                            tmp_file_path.string())
                    .c_str());

    out.write(data.data(), data.size());
    HOST_ASSERT(out.good(),
                fmt::format("Failed to write data to temporary file: {}. "
                            "Stream state: fail={}, bad={}",
                            tmp_file_path.string(), out.fail(), out.bad())
                    .c_str());
    out.close();
  ...
  std::shared_ptr<KernelRuntime> build(const std::string &name,
    ...
    if (const auto &runtime_ptr = jit_cache->get_runtime(full_path);
        runtime_ptr != nullptr) {
      if (kernel2_trace_host_enabled()) {
    ...
    // once write into tmp_path is complete, with a fully written binary, we
    // need to rename it

    fs::rename(tmp_path, full_path / "kernel.cubin");
    // also need to populate with kernel.cu

    std::shared_ptr<KernelRuntime> new_runtime =
        std::make_shared<KernelRuntime>(full_path);
...
public:
  NVCC_Compiler() {
    ...
    HOST_WARNING(major == 12 && minor >= 9,
                 "Warning: moe_cuda was built with NVCC 12.9, use 12.9 for "
                 "better performance");
    signature = fmt::format("NVCC-{}-{}", major, minor);
    const auto arch = device_prop->get_major_minor();

    // ThunderKittens include paths: TK_ROOT override, then common repo-relative
    // candidates.
    ...
    tk_candidates.push_back(library_root_path / "third-party" /
                            "ThunderKittens");
    tk_candidates.push_back(library_root_path.parent_path() / "third-party" /
                            "ThunderKittens");
    for (const auto &tk_root : tk_candidates) {
      if (fs::exists(tk_root / "include" / "kittens.cuh") &&
        ...
        extra_includes = fmt::format("-I{} -I{} -DKITTENS_HOPPER ",
                                     (tk_root / "include").string(),
                                     (tk_root / "prototype").string());
        break;
      }
    }
    ...
    const auto compiler_bindir =
        (gcc_exit == 0 && !gcc_path.empty())
  ...
  void compile(const std::string &code, const fs::path kernel_dir_path,
    ...
    std::string command =
        fmt::format("{} {} -o {} {}", nvcc_path.string(), code_path.string(),
...
public:
  NVRTCCompiler() {
    ...
    if (!tk_found && get_env<int>("JIT_DEBUG", 0)) {
      printf("Warning: ThunderKittens headers not found for NVRTC include "
             "paths\n");
    }
    ...
    flags = fmt::format(
        "{} {} --gpu-architecture=sm_{} -default-device {} --diag-suppress=639",
        flags, include_dirs, device_prop->get_arch(major > 12 || minor >= 9),
        pch_flags);
  }
  ...
  void compile(const std::string &code, const fs::path kernel_dir_path,
    ...
    NVRTC_CHECK(nvrtcCreateProgram(&program, code.c_str(), "kernel.cu", 0,
                                   nullptr, nullptr));
