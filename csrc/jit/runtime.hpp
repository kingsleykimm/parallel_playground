...
public:
  ...
  // kernel runtime is created from the current cached_dir, which contains the
  // compiled cuda binary that is converted into a CuFunction
  KernelRuntime(const fs::path cached_dir) {
    ...
    // need to extract the symbol name of the function that is the kernel
    // entrypoint, and extract that
    const std::vector<std::string> filters = {"vprintf", "__instantiate_kernel",
                                              "__internal", "__assertfail"};
    auto command =
        fmt::format("{} -symbols {}", cuobjdump_path.string().c_str(),
    ...
    for (std::string line; std::getline(stream, line);) {
      if (line.find("STT_FUNC") != std::string::npos &&
          ...
          std::none_of(filters.begin(), filters.end(), [&](const auto &name) {
            // condition to check that none of the filters are inside the symbol
            // name
    ...
    HOST_ASSERT(symbol_names.size() > 0,
                "Symbol names were not found in cuobjdump");
    CUDA_CHECK(cuModuleLoad(&library, cubin_path.c_str()));
    CUDA_CHECK(
        cuModuleGetFunction(&kernel_handle, library, symbol_names[0].c_str()));
  }
...
// this allows compile-time polymorphism, since derived class implement the
// exposed base methods
template <class Derived> class LaunchRuntime {
...
public:
  ...
  // this is a wrapper function around the args, pass down the actual
  // implementation logic to the derived
  template <typename Args>
  static void launch(std::shared_ptr<KernelRuntime> &kernel_runtime,
                     const Args &args) {
