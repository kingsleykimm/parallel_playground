...
__host__ constexpr TA constexpr_host_ceil_div(TA a, TB b) {
  return (a + b - 1) / b;
}

// align a to b
template <typename TA, typename TB>
__host__ constexpr TA constexpr_host_align(TA a, TB b) {
...
inline int get_swizzle_mode(const uint32_t &block_size,
  ...
  std::string msg = "There does not exist a compatible swizzle mode for the "
                    "current block size " +
                    std::to_string(block_size) + " and elem_size " +
...
static CUtensorMapL2promotion
getCuL2PromotionSize(const int &l2_promotion_size) {
...
static CUtensorMap make_tma_2d_desc(at::Tensor &t,
                                    ...
                                    size_t gmem_outer_dim,
                                    size_t gmem_outer_stride,
                                    size_t smem_inner_dim, // faster dimension
                                    size_t smem_outer_dim,
                                    const int &swizzle_size,
                                    const int &l2_promotion_size,
                                    CUtensorMapFloatOOBfill_enum fill_mode =
  ...
  const cuuint64_t gmem_strides[1] = {
      static_cast<cuuint64_t>(gmem_outer_stride * type_size)};
  ...
  if (get_env<int>("JIT_DEBUG")) {
    printf("Making TMA desc: global memory: %zu %zu, shared memory: %zu %zu, "
           "outer stride: %zu, swizzle: %d elem size: %zu\n",
  ...
  CUDA_CHECK(cuTensorMapEncodeTiled(
      &map, convert_to_cudtype(dtype_of(t)), 2, t.data_ptr(), gmem_dims,
      gmem_strides, smem_dims, elem_strides, CU_TENSOR_MAP_INTERLEAVE_NONE,
      swizzle_type, getCuL2PromotionSize(l2_promotion_size), fill_mode));
  return map;
}

static CUtensorMap make_tma_3d_desc(at::Tensor &t, size_t gmem_dim_0,
                                    size_t gmem_dim_1, size_t gmem_dim_2,
                                    size_t gmem_stride_1, size_t gmem_stride_2,
                                    size_t smem_dim_0, size_t smem_dim_1,
                                    size_t smem_dim_2, const int &swizzle_size,
                                    const int &l2_promotion_size) {
  HOST_ASSERT(t.data_ptr() != nullptr, "Tensor data is null");
  size_t type_size = get_type_size(dtype_of(t));
  if (swizzle_size > 0) {
  ...
  const cuuint64_t gmem_strides[2] = {
      static_cast<cuuint64_t>(gmem_stride_1 * type_size),
  ...
  CUtensorMapSwizzle swizzle_type = getCuSwizzle(swizzle_size);
  if (get_env<int>("JIT_DEBUG")) {
    printf("Making TMA desc: global memory: %zu %zu %zu, shared memory: %zu "
           "%zu %zu, outer stride: %zu %zu, swizzle: %d elem size: %zu \n",
           gmem_dim_0, gmem_dim_1, gmem_dim_2, smem_dim_0, smem_dim_1,
           smem_dim_2, gmem_stride_1, gmem_stride_2, swizzle_size, type_size);
  }
  CUDA_CHECK(cuTensorMapEncodeTiled(
      &map, convert_to_cudtype(dtype_of(t)), 3, t.data_ptr(), gmem_dims,
      gmem_strides, smem_dims, elem_strides, CU_TENSOR_MAP_INTERLEAVE_NONE,
      swizzle_type, getCuL2PromotionSize(l2_promotion_size),
      CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE));
  return map;
}
static CUtensorMap
make_tma_4d_desc(at::Tensor &t, size_t gmem_dim_0, size_t gmem_dim_1,
  ...
  const cuuint64_t gmem_strides[3] = {
      static_cast<cuuint64_t>(gmem_stride_1 * type_size),
      static_cast<cuuint64_t>(gmem_stride_2 * type_size),
  ...
  if (get_env<int>("JIT_DEBUG")) {
    printf(
        "Making TMA desc: global memory: %zu %zu %zu %zu, shared memory: %zu "
        "%zu %zu %zu, outer stride: %zu %zu %zu, swizzle: %d elem size: %zu \n",
        gmem_dim_0, gmem_dim_1, gmem_dim_2, gmem_dim_3, smem_dim_0, smem_dim_1,
        smem_dim_2, smem_dim_3, gmem_stride_1, gmem_stride_2, gmem_stride_3,
        swizzle_size, type_size);
  }
  CUDA_CHECK(cuTensorMapEncodeTiled(
      &map, convert_to_cudtype(dtype_of(t)), 4, t.data_ptr(), gmem_dims,
      gmem_strides, smem_dims, elem_strides, CU_TENSOR_MAP_INTERLEAVE_NONE,
      swizzle_type, getCuL2PromotionSize(l2_promotion_size),
      CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE));
  return map;
...
static CUtensorMap make_tma_a_desc(at::Tensor &t, Major major,
                                   uint32_t num_groups, uint32_t block_m,
                                   uint32_t block_k, uint32_t outer_stride,
                                   const int &swizzle_size,
                                   const int &l2_promotion_size) {
  uint32_t global_m = t.size(-2);
  uint32_t global_k = t.size(-1);
  auto [gmem_inner_dim, gmem_outer_dim] =
      get_inner_outer_dims(major, global_m * num_groups, global_k);
  auto [smem_inner_dim, smem_outer_dim] =
      get_inner_outer_dims(major, block_m, block_k);
  return make_tma_2d_desc(t, gmem_inner_dim, gmem_outer_dim, outer_stride,
                          smem_inner_dim, smem_outer_dim, swizzle_size,
                          l2_promotion_size);
}

static CUtensorMap make_tma_a_desc_3d(at::Tensor &t, Major major,
                                      uint32_t num_groups, uint32_t block_m,
                                      uint32_t block_k, const int &swizzle_size,
                                      const int l2_promotion_size) {
  uint32_t global_m = t.size(-2);
  uint32_t global_k = t.size(-1);
  auto [gmem_inner_dim, gmem_outer_dim] =
      get_inner_outer_dims(major, global_m, global_k);
  auto [smem_inner_dim, smem_outer_dim] =
      get_inner_outer_dims(major, block_m, block_k);
  ...
  return make_tma_3d_desc(t, gmem_inner_dim, gmem_outer_dim, num_groups,
                          outer_stride, batch_stride, smem_inner_dim,
                          smem_outer_dim, 1, swizzle_size, l2_promotion_size);
}

static CUtensorMap make_tma_b_desc_3d(at::Tensor &t, Major major,
                                      uint32_t num_groups, uint32_t block_n,
                                      uint32_t block_k, const int &swizzle_size,
                                      const int l2_promotion_size) {
  uint32_t global_n = t.size(-2);
  uint32_t global_k = t.size(-1);
  auto [gmem_inner_dim, gmem_outer_dim] =
      get_inner_outer_dims(major, global_n, global_k);
  auto [smem_inner_dim, smem_outer_dim] =
      get_inner_outer_dims(major, block_n, block_k);
  auto batch_stride = t.stride(0);
  auto outer_stride = major == Major::K ? t.stride(-2) : t.stride(-1);
  return make_tma_3d_desc(t, gmem_inner_dim, gmem_outer_dim, num_groups,
                          outer_stride, batch_stride, smem_inner_dim,
...
static CUtensorMap make_tma_d_desc_3d(at::Tensor &t, Major major,
                                      uint32_t num_groups, uint32_t block_m,
                                      uint32_t block_n, const int &swizzle_size,
                                      const int l2_promotion_size) {
  uint32_t global_m = t.size(-2);
  uint32_t global_n = t.size(-1);
  auto [gmem_inner_dim, gmem_outer_dim] =
      get_inner_outer_dims(major, global_m, global_n);
  auto [smem_inner_dim, smem_outer_dim] =
      get_inner_outer_dims(major, block_m, block_n);
  auto batch_stride = t.stride(0);
  auto outer_stride = major == Major::K ? t.stride(-2) : t.stride(-1);
  return make_tma_3d_desc(t, gmem_inner_dim, gmem_outer_dim, num_groups,
                          outer_stride, batch_stride, smem_inner_dim,
...
// static CUtensorMap make_tma_batch_a_desc(at::Tensor& t, Major major, int
// batch_size, int num_groups, int block_m, int block_k, int outer_stride, const
// int& swizzle_size) {
//     // outer stride is always the outermost stride, for batched we can assume
//     that the stride_1 is fixed by majorness int global_m = t.size(1); int
//     global_k = t.size(2); auto [gmem_inner_dim, gmem_outer_dim] =
//     get_inner_outer_dims(major, global_m, global_k); auto [smem_inner_dim,
//     smem_outer_dim] = get_inner_outer_dims(major, block_m, block_k); return
//     make_tma_3d_desc(t, gmem_inner_dim, gmem_outer_dim * num_groups,
//     batch_size,
//       gmem_inner_dim, outer_stride, smem_inner_dim, smem_outer_dim,
//       smem_inner_dim * smem_outer_dim, swizzle_size);
// }

static CUtensorMap make_tma_b_desc(at::Tensor &t, Major major,
                                   uint32_t num_groups, uint32_t block_n,
                                   uint32_t block_k, uint32_t outer_stride,
                                   const int &swizzle_size,
                                   const int &l2_promotion_size) {
  ...
  auto [smem_inner_dim, smem_outer_dim] =
      get_inner_outer_dims(major, block_n, block_k);
  return make_tma_2d_desc(t, gmem_inner_dim, gmem_outer_dim * num_groups,
                          outer_stride, smem_inner_dim, smem_outer_dim,
                          swizzle_size, l2_promotion_size);
}

static CUtensorMap make_tma_d_desc(at::Tensor &t, Major major,
                                   uint32_t num_groups, uint32_t block_m,
                                   uint32_t block_n, uint32_t outer_stride,
                                   const int &swizzle_size,
                                   const int &l2_promotion_size) {
  uint32_t global_m = t.size(-2);
  uint32_t global_n = t.size(-1);
...
static CUtensorMap make_tma_sf_desc(at::Tensor &t, Major major, int num_groups,
                                    int global_mn, int global_k, int block_mn,
                                    int block_k) {
  if (major != Major::MN) {
  ...
  // global_k is already in scale-block units (K/block_k for FP32, K/(block_k/4)
  // for NVFP4) so we should NOT divide by block_k again
  return make_tma_2d_desc(
      t, aligned_global_mn,
      host_ceil_div(global_k, block_k) *
          (dtype_of(t) == c10::ScalarType::Float ? 1 : 4) *
          num_groups, // already in k_blocks, just multiply by num_groups
      aligned_global_mn, block_mn, 1, 0, 0);
}

// prepare chunk indices for chunked varlen kernels
inline std::vector<int>
prepare_chunk_indices(const std::vector<int> &cu_seqlens, int chunk_size = 64) {
  // find diffs first
...
inline std::vector<int> prepare_cu_chunks(const std::vector<int> &cu_seqlens,
                                          int chunk_size = 64,
