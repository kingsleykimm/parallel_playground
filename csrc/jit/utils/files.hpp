...
static std::vector<fs::path> all_files_in_dir(
...
inline fs::path make_dir(const fs::path & file_path) {
    ...
    if (!created && ec) {
        HOST_ASSERT(false, fmt::format("Failed to make directory {}, with error code {}", 
                (file_path).string(), ec.value()).c_str());
    }
    return file_path;
}
...
static std::string get_uuid() {
    ...
    // to generate an uuid, use one random number, the process id and the current time
...
static std::pair<int, std::string> run_command(
    std::string command
) {
    // combine stderr and stdout first, stderr > &stdout
    command += " 2>&1";
    // pipe cleanup function
    const auto& cleanup = [&](FILE* f) {if (f) pclose(f); };
