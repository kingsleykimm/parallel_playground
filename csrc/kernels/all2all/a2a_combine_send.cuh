...
template <size_t kNumThreads, uint32_t NODE_SIZE, uint32_t DP_SIZE,
          typename TokenDim_t>
__global__ void __launch_bounds__(kNumThreads, 1)
    a2a_combine_send_kernel(const size_t token_dim, const size_t rank,
...
#pragma unroll NUM_STAGES
  ...
  if (warp_id == 0) {
    uint32_t next_token =
        token + lane_id * gridDim.x; // blockIdx.x + lane_id * gridDim.x, this
                                     // is blockwide but we're only filling the
                                     // first NUM_STAGES at each block iteration
  ...
  while (token < num_recv_tokens) {
    ...
    // pipelined copy, we already loaded into shared, now we process
    // local_stages information
    uint4 values[NUM_STAGES];

    for (uint32_t i = threadIdx.x; i * sizeof(uint4) < token_bound_fixed;
         i += kNumThreads) {
#pragma unroll NUM_STAGES
      for (uint32_t s = 0;
           s < NUM_STAGES && token + s * gridDim.x < num_recv_tokens; s++) {
        auto *ptr =
            (uint4 *)(expert_x_ptr + expert_x_stride * local_stages[s].index);
...
#pragma unroll DP_SIZE
          for (uint32_t dp_peer = 0; dp_peer < DP_SIZE;
               dp_peer++) { // cycle through ranks in dp group
            auto token_peer = (first_dp_rank + dp_peer) % NODE_SIZE;
            auto *x_token_dst = (uint4 *)(recv_ptrs_local[token_peer] +
                                          offset * token_bound_fixed);
    ...
    if (warp_id == 0 && ti_elect_one_sync()) {
      *sync_counter = counter + 1;
      ...
      if constexpr (NODE_SIZE > 1) {
        auto local_rank = rank % NODE_SIZE;
        if (lane_id < NODE_SIZE) {
          st_release_u32(&sync_ptrs[lane_id][local_rank + NODE_SIZE],
                         counter + 1);
        }
      }
    }
...
cudaError_t a2a_kernels::a2a_combine_send(
    size_t num_blocks, size_t hidden_dim,
    ...
    uint32_t *sync_counter, uint32_t **sync_ptrs, uint8_t **recv_ptrs,
    cudaStream_t stream) {
    ...
    LAUNCH_WORLD_SIZE(node_size, NODE_SIZE, {
      LAUNCH_TOKEN_DIM(token_dim, TokenDim_t, {
        status = cudaLaunchCooperativeKernel(
            (void *)&a2a_combine_send_kernel<kNumThreads, NODE_SIZE, DP_SIZE,
                                             TokenDim_t>,
