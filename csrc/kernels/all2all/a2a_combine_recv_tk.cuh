...
__device__ void kernel(globals<EXPERTS_PER_TOKEN, NUM_EXPERTS, TOKEN_DIM> &G) {
...

  {
    ...
    for (;;) {
      ...
      const uint32_t expert =
          G.indices[{token, route}]; // this uses the global expert, needed
  ...
  if (warp_id == 0 && config::NUM_DEVICES > 1) {
    if (lane_id < config::NUM_DEVICES) {
      node_sync::wait(G.barrier, {G.rank + config::NUM_DEVICES},
                      lane_id, // make sure all other ranks have signaled that
                               // G.rank is ready
                      counter);
    }
  }
