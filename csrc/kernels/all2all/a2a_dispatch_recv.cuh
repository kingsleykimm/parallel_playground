...
/*
This kernel checks the sync ptrs that the dispatch recv sent, and then copies from its recv pointers into its local
output rows, preparing for MoE
...
template <size_t kNumThreads, uint32_t NODE_SIZE, typename TokenDim_t, typename HiddenDimScale_t>
__global__ void __launch_bounds__(kNumThreads, 1)
    a2a_dispatch_recv_kernel(const size_t token_dim, const size_t token_scale_dim, const size_t token_stride,
                             size_t hidden_dim, size_t hidden_dim_scale, size_t x_elemsize, size_t x_scale_elemsize,
                             size_t num_experts, size_t rank, size_t world_size,
                             int32_t *__restrict__ out_num_tokens_ptr, std::byte *__restrict__ out_x_ptr,
                             size_t out_x_stride, float *__restrict__ out_x_scale_ptr, size_t out_x_scale_stride_elem,
                             ...
                             uint32_t *__restrict__ num_recv_tokens_ptr, uint32_t *__restrict__ sync_counter,
                             uint32_t **__restrict__ sync_ptrs, std::byte **send_ptrs) {
    ...
    if constexpr (NODE_SIZE > 1) {
      ...
      if (lane_id < NODE_SIZE) {
        ...
        while (ld_acquire_u32(flag_ptr) !=
               counter) // we're load_acquiring the store relase from the dispatch_recv, to ensure explicit memory fence
          ;
      }
    }
  }
  ...
  for (uint32_t token = blockIdx.x; token < num_recv_tokens; token += gridDim.x) {
    ...
    auto position = source_offset[token]; // source_dispatch_offset in worker.hpp, gives us absolute position

    uint4 *x_token_src;
    // copy from send buffer or send_ptrs
    if (token_rank == rank) {
      x_token_src = (uint4 *)(send_buffer + position * token_stride);
    } else if (position & (1u << 31)) { // nv link -> send buffer copy, but it's overflow over the max_private_tokens,
                                        // so we take from the peer's send buffer
      x_token_src = (uint4 *)(send_ptrs_local[local_rank] + (position & ~(1u << 31)) * token_stride);
    } else {
      // position < max_private_tokens, directly copied into our recv buffer from dispatch_send
    ...
    for (uint32_t i = threadIdx.x; i * sizeof(uint4) < token_dim; i += kNumThreads) {
  ...
  if (blockIdx.x == 0) {
    for (unsigned expert = threadIdx.x; expert < last_expert - first_expert; expert += blockDim.x) {
      out_num_tokens_ptr[expert] =
          tokens_per_expert[expert]; // copy the output counts per expert that were calculated in dispatch_send, this
                                     // tells the number, maybe for debugging or load balancing
    }
  }
  ...
  // structure:
  /*
  we're not doing RDMAs, but the original authors overlap nvlink (Faster) with the longer RDMA writes into recv_buffer
  we load in the number of received tokens from num_recv_tokens_ptr, once we know it is written into, and then we start
  the transfer

  we don't need to use the shared_to_local - just remember to put a 31st bit flag on the source_offset of a given token
  so we know it's from another rank
  ...
  then updates the counter and stores to sync_ptrs to ensure combine kernel won't overwrite
  */
}

cudaError_t a2a_kernels::a2a_dispatch_recv(size_t num_blocks, size_t hidden_dim, size_t hidden_dim_scale,
  ...
  const size_t token_scale_dim = ti_align(hidden_dim_scale * x_scale_elemsize, sizeof(float4));
    ...
    LAUNCH_TOKEN_DIM(token_dim, TokenDim_t, {
      LAUNCH_HIDDEN_DIM_SCALE(hidden_dim_scale, HiddenDimScale_t, {
        status = cudaLaunchCooperativeKernel(
            (void *)&a2a_dispatch_recv_kernel<kNumThreads, NODE_SIZE, TokenDim_t, HiddenDimScale_t>, dimGrid, dimBlock,
            args, 0, stream);
      });
    });
  });
