...
void fp8_gemm(
    ...
    int* grouped_layout,
    cudaStream_t& stream) {
    api::fp8_gemm(act, weight, output, gemm_type, compiled_dims, grouped_layout, stream);
}
...
void a2a_combine(
    ...
    bool accumulate,
    cudaStream_t stream) {
    all2all.combine(out_tokens, indices, weights, expert_y, bound_m, do_send, do_recv, accumulate, stream);
}
#endif
